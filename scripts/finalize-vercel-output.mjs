 ...
 * `preserveBuildServerDir` is on, and Astro 7.2.9 writes Monaco workers to
 * `client/` when `build.server` lives under `.vercel/output`. Neither directory
 * is part of the Build Output API. Preview deploys of Astro 7.2.6 + adapter
 * 11.0.8 still failed after CSP was collapsed; 7.2.0 + 11.0.5 (no leftover
 * dirs) succeeded. Merge `client/` into `static/` so `/monaco` workers are
 * served, then drop the leftovers.
 *
