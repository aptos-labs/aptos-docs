...
await build({
  ...
  build: {
    ...
    emptyOutDir: false,
    lib: {
      entry: "src/vercel-edge-middleware.ts",
      fileName: "middleware",
      formats: ["es"],
    },
    minify: false,
...
function updateAST(ast) {
  ...
  for (let node of ast.body) {
    ...
    if (node.type === "ExportNamedDeclaration") {
      const defaultSpecifier = node.specifiers.find(
        (s) => s.type === "ExportSpecifier" && s.exported?.value === "default",
      );

      if (defaultSpecifier) {
        middlewareIdentifier = defaultSpecifier.orig;
      }
  ...
  if (middlewareIdentifier) {
    newBody.push({
      type: "ExportDefaultExpression",
      span: DUMMY_SPAN,
      expression: middlewareIdentifier,
    });
  }
