---
title: "Indexador"
description: "Consulta datos indexados de blockchain usando GraphQL API, crea procesadores personalizados con el SDK del Indexador, o transmite transacciones en bruto desde la blockchain Aptos"
sidebar:
  label: "Indexador"
---
...
1. Consulta la [API del Indexador](/es/build/indexer/indexer-api) para obtener datos básicos sobre transacciones, activos fungibles y tokens
2. Indexa tu contrato personalizado con el [SDK del Indexador](/es/build/indexer/indexer-sdk)
3. Transmite transacciones sin procesar desde el [Servicio de Transmisión de Transacciones](/es/build/indexer/txn-stream) a tu procesador o servicio
...
3. Datos que son difíciles de obtener desde la API más simple del [Nodo de Aptos](/es/build/apis/fullnode-rest-api) (ej. ¿Qué cuenta posee un token llamado "ExampleToken"?).
...
Si la API del Indexador hospedada no es suficiente o si quieres indexar tu contrato personalizado, puedes crear un procesador con el [SDK del Indexador](/es/build/indexer/indexer-sdk).
...
Encuentra información sobre el indexador legacy [aquí](/es/build/indexer/legacy).
