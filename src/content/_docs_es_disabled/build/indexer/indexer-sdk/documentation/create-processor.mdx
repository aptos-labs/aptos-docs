...
Si no, sigue la [guía de instalación del SDK de Indexer](/es/build/indexer/indexer-sdk/documentation/setup).
...
Debería incluir configuración para cosas que se comparten a través de múltiples procesadores, como la configuración de base de datos y configuración de [Transaction Stream](/es/build/indexer/txn-stream).
...
Aquí también implementamos lógica para determinar la versión de inicio apropiada para el procesador, verificar el ID de cadena usando [Transaction Stream](/es/build/indexer/txn-stream), y validar la configuración del procesador.
...
Como referencia, también puedes ver todos los procesadores que componen la [API de Indexer](/es/build/indexer/indexer-api) en [`aptos-indexer-processors`](https://github.com/aptos-labs/aptos-indexer-processors-v2/tree/main/processor/src/processors).
