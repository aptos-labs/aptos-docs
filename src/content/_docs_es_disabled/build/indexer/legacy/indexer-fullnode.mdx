...
<Aside type="caution">
  Esta es documentación para el indexer legacy. Para aprender cómo ejecutar la infraestructura subyacente para la pila de indexer más reciente, ver [Transaction Stream Service](/es/build/indexer/txn-stream).
</Aside>
...
2. Seguir las instrucciones para [configurar un fullnode público](/es/network/nodes/full-node/verify-pfn) pero no iniciar el fullnode aún.
...
1. Seguir las instrucciones para configurar un [fullnode público](/es/network/nodes/full-node/verify-pfn) y preparar la configuración, pero **no** iniciar aún el indexer (con `cargo run` o `docker run`).
