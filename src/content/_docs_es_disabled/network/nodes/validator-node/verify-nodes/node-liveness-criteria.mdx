...
de las métricas importantes, ver la documentación del [Servicio de Inspección de Nodos](/es/network/nodes/measure/node-inspection-service) y
...
- **Métricas:** Puedes monitorear tu endpoint de métricas local ejecutando un comando `curl` contra el
  [Servicio de Inspección de Nodos](/es/network/nodes/measure/node-inspection-service) y verificar métricas clave. Por ejemplo, puedes
  verificar el estado de sincronización de tu nodo ejecutando el comando delineado en la
  sección [Verificar sincronización](/es/network/nodes/full-node/verify-pfn#verify-synchronization).
...
- **API REST:** También puedes monitorear la salud de tu nodo consultando la API REST. Por ejemplo, puedes verificar la
  ...
  [Especificación de la API de Aptos](/es/build/apis/fullnode-rest-api).
...
El equipo de Aptos Labs también puede monitorear tu nodo remotamente usando [Telemetría de Nodo](/es/network/nodes/configure/telemetry). Cuando habilitas telemetría en
