...
[DNS-AID](https://datatracker.ietf.org/doc/draft-mozleywilliams-dnsop-dnsaid/) records cannot be published from this repository. The `aptos.dev` zone is hosted on Google Cloud DNS (`ns-cloud-c1.googledomains.com` through `ns-cloud-c4.googledomains.com`). To pass DNS-based agent discovery, operators should add a ServiceMode [HTTPS](https://www.rfc-editor.org/rfc/rfc9460) record at the organization index and keep DNSSEC validating (the zone already has `DNSKEY` records; a `DS` record must also exist at the `.dev` parent):

```dns
_index._agents.aptos.dev. 3600 IN HTTPS 1 aptos.dev. alpn="h2,h3" port=443
```

That record points agents at `https://aptos.dev`, where the MCP Server Card, API catalog, and other well-known documents already live. Do not publish `_a2a._agents` or `_mcp._agents` until Aptos hosts those protocols on HTTPS — the MCP server is stdio (`npx @aptos-labs/aptos-mcp`), not a remote MCP endpoint.
