...
description: "Learn about AIP-115 which introduces stateless accounts that operate without explicitly created Account resources"
sidebar:
  label: "AIP-115"
---

[AIP-115](https://github.com/aptos-foundation/AIPs/blob/main/aips/aip-115-stateless-account-update.md) covers
stateless accounts.
...
It defaults to `0` if the account resource does not exist. [Orderless
transactions](/build/aips/aip-123) use a nonce instead of a sequence number, so
an account can stay without an `Account` resource.
...
Yes. Orderless transactions and stateless accounts are complementary. Orderless
transactions do not need a sequence number, so an account can stay without an
`Account` resource. See the [AIP-123 FAQ](/build/aips/aip-123).
