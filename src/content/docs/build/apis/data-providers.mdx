...
[REST API](../apis.mdx#aptos-fullnode) allows you to query the full node directly and will have the latest data (historical data will be missing unless it's an archival full node).

[GRPC transaction stream](../indexer/txn-stream/aptos-hosted-txn-stream.mdx) is a stream layer we built that serves typed version of above data

[GraphQL](../indexer/) is an endpoint where we provide product tables (such as transfers and balances) that can be queried

Product tables are parsed out transaction and logic is [public](https://github.com/aptos-labs/aptos-indexer-processors-v2),
...
On chain data is stored as: [table items](../smart-contracts/table.mdx), [resources](../../network/blockchain/resources.mdx) or [modules](../smart-contracts/modules-on-aptos.mdx) (executable)
...
Additional RPC vendors can be found [here](https://aptosfoundation.org/ecosystem/projects/rpc)
