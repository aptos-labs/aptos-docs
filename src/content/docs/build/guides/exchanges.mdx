...
description: "Comprehensive guide for integrating Aptos and its assets into cryptocurrency exchanges with balance tracking and testing."
sidebar:
  label: "Exchanges"
---
...
[AIP-115](/build/aips/aip-115) treats every 32-byte address as a valid account. You do **not** need to create an `0x1::account::Account` resource before sending funds or submitting a transaction.

The resource is created on demand when a sequence-number transaction (or another feature that needs it) first requires it. `0x1::aptos_account::transfer` still funds the recipient and will create the resource if needed. `0x1::aptos_account::create_account` remains available if you want to create the resource explicitly.

Do not treat a missing `0x1::account::Account` resource as "this address does not exist." Check balances and on-chain activity instead. See [Application Integration](/build/guides/application-integration) for the same guidance.

For hot wallets that submit from many machines without coordinating sequence numbers, use [orderless transactions](/build/guides/orderless-transactions).
