...
3. Build a transaction to transfer APT
...
<Tabs>
  ...
  <TabItem label="Python">
    In blockchain, all transactions must come from an account. Let's create two test accounts: one to send APT (Alice) and one to receive them (Bob).
    ...
    <Steps>
      1. Set up the client
         ...
         <Aside type="note">
           This walkthrough uses the Python SDK v1 API (`RestClient`). New projects should prefer the v2 API (`aptos_sdk.v2`). See the [Python SDK](/build/sdks/python-sdk) page and the [v1 to v2 migration guide](https://github.com/aptos-labs/aptos-python-sdk/blob/main/MIGRATION.md).
         </Aside>
...
<Tabs>
  <TabItem label="TypeScript">
    Now that we have funded accounts, let's create a transaction to transfer APT from Alice to Bob. This is like filling out a form specifying what you want to send and to whom.
    ...
    <Steps>
      1. Understand transaction structure
         ...
         2. **Function**: The on-chain function to call (in this case, an APT transfer)
         ...
         6. **Sequence number**: A counter that prevents replay attacks. For parallel submission from multiple machines, Aptos also supports [orderless transactions](/build/guides/orderless-transactions) that use a nonce instead.
      ...
      2. Build the transaction
         ...
         <Aside type="note">
           The function `0x1::aptos_account::transfer` is a built-in framework function that transfers APT between accounts. The `0x1` prefix marks it as part of the core framework. After the Fungible Asset migration, this entry function deposits APT into the recipient's primary fungible store rather than the legacy `CoinStore`. See [`aptos_account`](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-framework/sources/aptos_account.move) and the [Fungible Asset standard](/build/smart-contracts/fungible-asset).
         </Aside>
    </Steps>
  </TabItem>
