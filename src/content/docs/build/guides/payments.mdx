...
1. **[Sponsored](/build/guides/sponsored-transactions)** — another account pays gas. Build with `withFeePayer: true`; the sender `.sign`s and the sponsor `.signAsFeePayer`. Same on-chain cost; the fee payer is charged instead of the sender. [Geomi Gas Station](https://geomi.dev/docs/gas-stations) is a hosted fee payer.
