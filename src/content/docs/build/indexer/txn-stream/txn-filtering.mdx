...
Transaction filtering are applied by including them in the gRPC request to the transaction stream.
...
All filters are validated before use. A filter must have at least one field set:

- **TransactionRootFilter**: Must specify `success` or `txn_type`
- **UserTransactionFilter**: Must specify `sender` or `payload`
- **EventFilter**: Must specify `struct_type` or `data_substring_filter`
- **EntryFunctionFilter**: Must specify `address`, `module`, or `function`
- **MoveStructTagFilter**: Must specify `address`, `module`, or `name`

Invalid filters will return an error when validated or applied.
