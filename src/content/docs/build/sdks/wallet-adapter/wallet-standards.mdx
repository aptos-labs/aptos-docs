...
<Aside type="caution" title="Legacy wallet APIs have been removed">
  Direct wallet access via browser globals (such as `window.aptos` or `window.petra`) has been **removed**. These legacy APIs are no longer available in modern wallet versions (e.g. Petra has fully removed them). All dapps must use the [Aptos Wallet Adapter](/build/sdks/wallet-adapter/dapp) with the [AIP-62](https://github.com/aptos-foundation/AIPs/blob/main/aips/aip-62.md) standard for wallet integration.
</Aside>
