...
description: "Guidelines for wallet interoperability ensuring consistent dapp integration across all Aptos wallet types"
sidebar:
  label: "Wallet Standards"
---

import { Aside } from '@astrojs/starlight/components';

<Aside type="caution" title="Legacy wallet APIs are deprecated">
  Direct wallet access via browser globals (such as `window.aptos` or `window.petra`) is **deprecated** and should not be used. These legacy APIs are no longer maintained and may be removed from wallets at any time. All dapps should use the [Aptos Wallet Adapter](/build/sdks/wallet-adapter/dapp) with the [AIP-62](https://github.com/aptos-foundation/AIPs/blob/main/aips/aip-62.md) standard for wallet integration.
</Aside>
