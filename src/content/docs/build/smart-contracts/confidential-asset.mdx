...
For every asset type a user registers for, they generate a unique keypair:
...
Governance configures two auditor roles on-chain:

- **Global auditor** — set by governance; applies to every asset type unless an asset-specific auditor overrides it.
- **Asset-specific auditor** — set by governance per asset type and takes precedence over the global auditor.

The **effective auditor** is derived from those roles: the asset-specific auditor if its encryption key (`EK`) is set, otherwise the global auditor.
If an effective auditor exists, all transfers, withdrawals, and normalizations must include ciphertext components for it.

**Voluntary auditors** are optional additional auditors whose encryption keys the sender supplies at transfer time.
They receive ciphertext for the transfer amount only (not the sender's new available balance).
