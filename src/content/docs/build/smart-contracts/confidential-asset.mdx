...
The pending balance can only accumulate up to $2^{16}$ transfers before requiring a **rollover** into the available balance.
...
- Splitting the the encrypted value into 16-bit chunks.
...
<Aside type="caution">
  Registration is expensive as it allocates new storage slots, which is more expensive that the verification of the $\Sigma$-protocol proof.
...
This is necessary to do if the user has received too many incoming transfers, including [deposits](#deposit).
...
<Aside type="caution">
  ...
  Developers should use the [TypeScript SDK](/build/sdks/ts-sdk/confidential-asset) for this, which will generate all theses on the user's behalf.
</Aside>
...
Enables or disables confidential transfers for a specific asset type. Only meaningful when the allow list is enabled.
