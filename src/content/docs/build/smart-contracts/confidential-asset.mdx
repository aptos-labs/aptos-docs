...
Users can establish a confidential balance for an asset type by **registering** a `ConfidentialStore` for that asset type.

Specifically, they generate a unique keypair:

- An **encryption key (EK)** for that asset type that will be stored in its `ConfidentialStore`.
- A **decryption key (DK)** to be securely managed by the user.
...
- **pending balance** - accumulates all incoming transactions.
- **available balance** - used exclusively for outgoing transactions.
...
The `ConfidentialStore` is instantiated for each asset type the user has registered for and is managed by the `confidential_asset` module:
...
We discuss the `auditor_hint` field [later on](#auditors).
...
| Confidential balance type | Purpose                     | Chunks     | Max chunk size | Max encoded value |
| ------------------------- | --------------------------- | ---------- | -------------- | ----------------- |
| pending                   | receives incoming transfers | 4 × 16-bit | 32 bits        | $2^{64} - 1$      |
| available                 | funds outgoing transfers    | 8 × 16-bit | 32 bits        | $2^{128} - 1$     |

A user's _available balance_ is the source of funds for sending outgoing confidential transfers.

In contrast, a user's _pending balance_ is where all incoming confidential transfers accumulate in.
The pending balance can accumulate up to $2^{16}$ transfers before requiring a **rollover** into the available balance.
At such a point, pending balance chunks can grow up to 32 bits each.
Our design ensures that after rolling over such large chunks into the available balance, the resulting available balance chunks also remain 32-bit.

After a rollover, the available balance will have to be [normalized](#normalization) to support further rollovers: i.e., its potentially-large ($\le 32$-bit) chunks must be _converted_ back to 16-bit chunks.
In general, frequent normalization maintains very efficient decryption, although at a certain additional cost in gas.
...
Without going deep into the details, encryption involves:

- Splitting the transferred amount into 16-bit chunks.
...
Normalization ensures available balance chunks are always reduced to manageable sizes (16-bits), to ensure decryption is as fast as possible and to allow for further rollovers.

Without normalization, chunks can grow too large, making the decryption process (i.e., solving DL) significantly slower or even impractical.

Transfers and withdrawals update the available balance in a way that leaves it normalized automatically.
Users only need to normalize their available balance [manually](#normalize) if both conditions hold:

1. It is not already normalized
2. They need to call
...
The [TypeScript SDK](/build/sdks/ts-sdk/confidential-asset) can submit a normalization transaction automatically when a decryption key is provided.
...
1. decrypt transfer amounts from confidential transfers, which now additionally encrypt the transferred amount for the _effective auditor_ (and for the _voluntary auditors_)
...
- **Dispatchable FAs not supported.** Only non-dispatchable Fungible Assets can be used with CA. FAs with custom `withdraw`/`deposit`/`balance`/`supply` dispatch functions are rejected at registration.
