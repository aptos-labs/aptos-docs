...
Specifically, it allows users to establish a **confidential (encrypted) balance** and later **confidentially-transfer** <u>hidden</u> amounts from their confidential balance to another user's confidential balance.
...
| Balance type        | Purpose                                  | Chunks     | Max chunk size | Max encoded value |
| ------------------- | ---------------------------------------- | ---------- | -------------- | ----------------- |
| `pending_balance`   | receives incoming confidential transfers | 4 × 16-bit | 32 bits        | $2^{64} - 1$      |
| `available_balance` | sends outgoing confidential transfers    | 8 × 16-bit | 32 bits        | $2^{128} - 1$     |
...
Users need to normalize the available balance [manually](#normalize) only when it is not already normalized and they want to call
[`rollover_pending_balance`](#rollover-pending-balance) (including `rollover_pending_balance_and_pause`), which requires a normalized available balance first.
