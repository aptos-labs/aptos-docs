...
Normally, developers of such applications would have to wait until their desired cryptographic functionality is implemented efficiently as a [Move native function](../../../build/smart-contracts/book/functions.mdx#native-functions) in the [Aptos Move framework](../../../network/blockchain/move).
...
As an example, a Move developer can implement the popular Boneh-Lynn-Shacham (BLS) signature scheme generically over **any** curve by using [type arguments](../../../build/smart-contracts/book/functions.mdx#type-parameters) for the curve type in their implementation:
