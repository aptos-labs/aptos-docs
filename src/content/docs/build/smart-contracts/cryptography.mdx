...
title: "Cryptography"
label: "Cryptography"
description: "Learn about cryptography for Move smart contract development on Aptos blockchain."
sidebar:
  label: "Cryptography"
---
...
3. [Elliptic Curve Arithmetic](#elliptic-curve-arithmetic) – Elliptic curves are one of the building blocks of advanced cryptographic primitives, such as digital signatures, public-key encryption or verifiable secret sharing. Supported curves include Ristretto255, BN254 and BLS12-381.
...
1. **Economic Gas Usage** – Striving to minimize gas costs for Move developers by implementing key primitives as [Move native functions](https://aptos-labs.github.io/move-book/functions.html#native-functions). For example, see the module for [BLS signatures over BLS12-381 elliptic curves](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-stdlib/sources/cryptography/bls12381.move).
...
Normally, developers of such applications would have to wait until their desired cryptographic functionality is implemented efficiently as a [Move native function](https://aptos-labs.github.io/move-book/functions.html#native-functions) in the [Aptos Move framework](/network/blockchain/move).
...
As an example, a Move developer can implement the popular Boneh-Lynn-Shacham (BLS) signature scheme generically over **any** curve by using [type arguments](https://aptos-labs.github.io/move-book/functions.html#type-parameters) for the curve type in their implementation:
...
If you need help converting your `snarkjs` Groth16 VK, proof and public inputs to the expected format for the Move verifier above, you can use these community tools:

- [snarkjs-to-aptos](https://github.com/zjma/snarkjs-to-aptos) — converts `snarkjs` outputs to Aptos verifier artifacts.
- [export-aptos-verifier](https://github.com/mysteryon88/export-aptos-verifier) — exports `snarkjs`-compatible outputs for both BN256 and BLS12-381 curves, including gnark, noname and arkworks workflows.
- [zk-aptos-examples](https://github.com/zk-examples/zk-aptos-examples) — contains examples with generated Aptos smart contracts.
