...
Checks for functions or groups of functions that recurse infinitely. Note that the linter cannot detect _conditional_ infinite recursion, only inconditional, so a flag is a definite error in the contract. If the function in question is called, the transaction in which it is called _will_ abort.
