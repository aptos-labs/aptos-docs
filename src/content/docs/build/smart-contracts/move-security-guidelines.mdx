...
description: "Learn about move security guidelines for Move smart contract development on Aptos blockchain."
sidebar:
  label: "Security Guidelines"
---
...
Generics can be used to define functions and structs over different input data types. When using them, ensure that the generic types are valid and what’s expected. [Read more](https://aptos-labs.github.io/move-book/generics-and-abilities.html) about generics.
...
[Read more](https://aptos-labs.github.io/move-book/generics-and-abilities.html) about abilities.
...
### Mutable References (`&mut`) passed to untrusted code

Passing `&mut T` across a trust boundary should be treated as an **access control decision**: `&mut T` is a capability that lets the callee change the value behind the reference.

This does **not** break module encapsulation—outside code still can’t access `T`’s private fields or pack/unpack `T`. The risk is **whole-value replacement**: if the callee can obtain _another_ `T` value (e.g. a different `FungibleAsset`), it can swap it into your `&mut T`, changing private fields “indirectly” without ever reading or writing them.

With `mem::swap` now public, this whole-value replacement becomes easy to express. This risk exists for normal cross-module calls and **with function values (callbacks)** in scenarios where callers supply the untrusted function value. See the [`mem::swap` discussion in AIP-105](https://github.com/aptos-foundation/AIPs/issues/529#issuecomment-2605266224).

#### Impact

If you validate an invariant (e.g. “this `FungibleAsset` is the token I expect”) and then pass `&mut` to untrusted code, you must assume the invariant may no longer hold afterward. This can enable **token substitution** attacks: the callee swaps a “good” asset out for a “worthless” one and tricks the caller into thinking it is the original asset.

#### Example Insecure Code

```move filename="mem_swap_insecure.move"
module example::payments {
    use aptos_framework::fungible_asset::{Self, FungibleAsset};
    use aptos_framework::primary_fungible_store;

    const EUSED_TOO_MUCH: u64 = 1;
    const EUNEXPECTED_ASSET: u64 = 2;

    fun check_metadata(asset: &FungibleAsset) {
        // Check it is the asset class the protocol expects.
        // e.g. assert!(fungible_asset::metadata(asset) == expected, EUNEXPECTED_ASSET);
    }

    public fun do_with_fa(
        user: address, asset: FungibleAsset, hook: |&mut FungibleAsset|
    ) {
        check_metadata(&asset); // check it is type we expect

        // Attacker-supplied function value receives `&mut FungibleAsset`.
        hook(&mut asset);

        // Previously, protocols assumed the asset "identity" (metadata) couldn't change here.
        // With `mem::swap` (or any future whole-value replacement API), that is not a safe assumption:
        // `asset` may now be a different asset class.

        assert!(fungible_asset::amount(&asset) > 10, EUSED_TOO_MUCH);
        // Get asset value
        let value = fungible_asset::amount(&asset);
        // BUG: without re-checking metadata here, this could now be a worthless asset.
        primary_fungible_store::deposit(@treasury, asset);
        // Mint the pool tokens to the user
        mint_to(user, value);
    }

    fun mint_to(_user: address, _value: u64) { /* ... */ }
}
...
module attacker::exploit {
    ...
    // In practice this would come from attacker-controlled state / a withdraw.
    fun worthless_asset(): FungibleAsset { /* ... */ }

    public fun exploit(good: FungibleAsset) {
        let junk = worthless_asset();
        payments::do_with_fa(
            @attacker,
            good,
            |asset| {
                let kept = junk;
                mem::swap(asset, &mut kept); // replace the entire value behind `asset`
                // `kept` now holds the original ("good") asset; deposit it back to the attacker.
                // `asset` now holds the worthless replacement.
                primary_fungible_store::deposit(@attacker, kept);
            }
        );
    }
}
...
- Don't pass `&mut` references to untrusted code.
- If you must cross a trust boundary, **re-validate invariants after the call** (or snapshot and enforce that critical fields did not change).
- Prefer restricting mutation-heavy APIs to trusted modules (`public(friend)`) instead of exposing them as `public`.

```move filename="mem_swap_secure.move"
module example::payments {
    use aptos_framework::fungible_asset::{Self, FungibleAsset};
    use aptos_framework::primary_fungible_store;
    ...
    public fun do_with_fa(
        ...
        hook(&mut asset);
        check_metadata(&asset); // re-check after untrusted mutation (catches `mem::swap` substitution)
...
[Read more](https://aptos-labs.github.io/move-book/primitive-types.html#operations) about operations.
...
[Read more](https://aptos-labs.github.io/move-book/functions.html#reentrancy-check) about the function-value reentrancy checks.
