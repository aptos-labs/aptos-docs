...
### Mutable References (`&mut`) passed to untrusted code (`mem::swap`)

Passing `&mut T` across a trust boundary should be treated as an **access control decision**: `&mut` is a capability that grants the callee write-access to `T`.

With `mem::swap` now public, untrusted code holding `&mut T` can sometimes do more than “mutate fields”: it can **replace the entire value** behind the reference by swapping it with another value of the same type. This can invalidate invariants that were previously assumed to be stable across an external call. This risk exists for normal cross-module calls and is **more likely with function values (callbacks)** because they make it easy for callers to supply untrusted code. See the [`mem::swap` discussion in AIP-105](https://github.com/aptos-foundation/AIPs/issues/529#issuecomment-2605266224).

Note: this does **not** break module encapsulation. Outside code still can’t access `T`’s private fields or pack/unpack `T`. The issue is **whole-value replacement**: if an attacker can obtain _another_ `T` value (e.g. a different `FungibleAsset`), `mem::swap` lets them exchange the entire value behind your `&mut T`, changing private fields “indirectly” without ever reading or writing them.
...
module example::payments {
  ...
  public fun do_with_fa(user: address, asset: FungibleAsset, hook: |&mut FungibleAsset|) {
    check_metadata(&asset); // check it is type we expect

    // Attacker-supplied function value receives `&mut FungibleAsset`.
    hook(&mut asset);

    // Previously, protocols sometimes assumed the asset "identity" (metadata) couldn't change here.
