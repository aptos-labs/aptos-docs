...
label: "Dependencies"
description: "Learn about third party dependencies for Move smart contract development on Aptos blockchain."
sidebar:
  label: "Dependencies"
---
...
Third party dependencies are external [modules](https://aptos-labs.github.io/move-book/modules-and-packages.html) that a controlled module interacts with. Typically, these external modules exist under different accounts.

## Multi-DEX router example

A multi-DEX router actively utilizes third party dependencies. Instead of submitting multiple transactions to interact with different DEXs and their individual [entry](https://aptos-labs.github.io/move-book/functions.html#entry-modifier) functions within a swap route, a module (or [script](https://aptos-labs.github.io/move-book/modules-and-packages.html)) can consolidate all independent DEX invocations into a single, atomic transaction. The multi-DEX router references and calls to functions present in each of the third party DEX modules to achieve this.
...
The Aptos CLI allows for downloading a [package's](https://aptos-labs.github.io/move-book/modules-and-packages.html) bytecode.
