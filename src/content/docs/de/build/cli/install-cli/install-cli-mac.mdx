...
<Aside type="caution">
  If `brew` does not work for you, you can try the steps here: [Über Skript installieren](#install-via-script) or [Über vorkompilierte Binärdateien installieren](#install-via-pre-compiled-binaries-backup-method).)
</Aside>
...
# Über Skript installieren

<Steps>
  1. Verwenden Sie im Terminal einen der folgenden Befehle:
...
# Über vorkompilierte Binärdateien installieren (Backup Method)
