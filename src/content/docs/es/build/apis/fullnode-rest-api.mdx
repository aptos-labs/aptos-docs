...
description: "API REST de bajo nivel para leer estado, enviar transacciones y simular operaciones en la blockchain de Aptos"
sidebar:
  label: "API REST"
---
...
Para consultas más avanzadas, recomendamos usar la [API GraphQL del Indexador](/es/build/indexer).
...
Como con el [Indexador de Aptos](/es/build/indexer/indexer-api), la API REST de Aptos tiene límites de tasa basados en unidades de cómputo. Puedes aprender más sobre cómo funciona el límite de tasa leyendo la [documentación de Geomi](https://geomi.dev/docs/admin/billing).
...
- A medida que las transacciones se ejecutan, pueden emitir eventos. Los [Eventos](/es/network/blockchain/events) son pistas sobre cambios en datos
  de la cadena.

<Aside type="note">
  Asegúrate de que el [nodo completo](/es/network/nodes/networks) con el que te estás comunicando esté actualizado. El nodo completo debe alcanzar la
  versión que contiene tu transacción para recuperar datos relevantes de ella. Puede haber latencia de los nodos completos
  recuperando estado de [nodos completos validador](/es/network/blockchain/fullnodes), que a su vez dependen de
  [nodos validador](/es/network/blockchain/validator-nodes) como la fuente de verdad.
</Aside>
...
Para usar las funciones View, necesitas [publicar el módulo](/es/build/cli/working-with-move-contracts)
a través de la [CLI de Aptos](/es/build/cli).
