...
<Steps>
  ...
  5. Espera a que la red se inicie
     ...
     - [Node API](/es/build/apis/fullnode-rest-api): Esta es una API REST que se ejecuta directamente en el nodo. Habilita funcionalidad de escritura central como envío de transacciones y un conjunto limitado de funcionalidad de lectura, como leer recursos de cuenta o información de módulos Move.
     - [Indexer API](/es/build/indexer/indexer-api): Esta es una API [GraphQL](https://graphql.org/) que proporciona acceso de lectura rico a datos indexados de blockchain. Si haces clic en la URL para la API del Indexer arriba, por defecto [http://127.0.0.1:8090](http://127.0.0.1:8090), abrirá la Consola Hasura, una interfaz web que te ayudará a consultar la API GraphQL del Indexer.
     - [Transaction Stream Service](/es/build/indexer/txn-stream): Este es un stream gRPC de transacciones usado por la API del Indexer y SDK. Esto solo es relevante para ti si estás desarrollando un procesador personalizado [Indexer SDK](/es/build/indexer/indexer-sdk).
     ...
     - [Faucet](/es/build/apis/faucet-api): Puedes usar esto para financiar cuentas en tu red local.
...
- Estás construyendo un procesador personalizado [Indexer SDK](/es/build/indexer/indexer-sdk) y te gustaría indexar usando una red fresca.
