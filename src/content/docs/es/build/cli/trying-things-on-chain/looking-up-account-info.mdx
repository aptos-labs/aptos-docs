---
title: "Buscar Información de Cuenta En Cadena Usando CLI de Aptos"
description: "Aprende cómo consultar información de cuenta en cadena incluyendo balances, recursos y módulos Move usando el CLI de Aptos con ejemplos detallados y explicaciones de salida."
sidebar:
  label: "Información de Cuenta"
---

import { Aside } from '@astrojs/starlight/components';

<Aside type="note" emoji="ℹ️">
  Puedes instalar el CLI de Aptos siguiendo [estos pasos](/es/build/cli) si no lo has hecho ya.
</Aside>

Puedes buscar recursos y datos que una cuenta tiene en cadena ejecutando el siguiente comando:

```shellscript filename="Terminal"
aptos account list --account <nombre-de-tu-perfil-o-direccion-de-cuenta>
```

Esto mostrará todos los recursos que una cuenta tiene. Por ejemplo, abajo muestra el balance como `coin:value`, y la moneda asociada para el token de gas nativo APT sería `0x1::aptos_coin::AptosCoin`.
Esto se representa en subdivisiones, así que en este caso son `10^-8` o 8 ceros de puntos decimales.

```json filename="Output"
{
  "Result": [
    {
      "coin": {
        "value": "110000"
      },
      "deposit_events": {
        "counter": "3",
        "guid": {
          "id": {
            "addr": "0xf1f20ddd0b0dd2291b6e42c97274668c479bca70f07c6b6a80b99720779696",
            "creation_num": "2"
          }
        }
      },
      "frozen": false,
      "withdraw_events": {
        "counter": "0",
        "guid": {
          "id": {
            "addr": "0xf1f20ddd0b0dd2291b6e42c97274668c479bca70f07c6b6a80b99720779696",
            "creation_num": "3"
          }
        }
      }
    }
  ]
}
```

Si estás interesado en un tipo específico de datos de cuenta, puedes especificar eso con el parámetro `--query`. Las consultas soportadas son:

- `balance` - para ver el balance actual y una lista de eventos de depósito y retiro.
- `modules` - ver los contratos Move que están publicados en esta cuenta.
- `resources` - esto es lo que hace el comando predeterminado sin consulta especificada.

Aquí hay un ejemplo de cómo se ve llamar con el parámetro `--query modules`:

```shellscript filename="Terminal"
aptos account list --query modules
```

Esto mostrará todos los módulos que una cuenta tiene. Por ejemplo:

```json filename="Output"
{
  "Result": [
    {
      "bytecode": "",
      "abi": {
        "address": "0xb9bd2cfa58ca29bce1d7add25fce5c62220604cd0236fe3f90d9de91ed9fb8cb",
        "name": "Message",
        "friends": [],
        "exposed_functions": [
          {
            "name": "get_message",
            "visibility": "public",
            "is_entry": false,
            "generic_type_params": [],
            "params": [
              "address"
            ],
            "return": [
              "0x1::string::String"
            ]
          },
          {
            "name": "set_message",
            "visibility": "public",
            "is_entry": true,
            "generic_type_params": [],
            "params": [
              "signer",
              "vector<u8>"
            ],
            "return": []
          }
        ],
        "structs": [
          {
            "name": "MessageChangeEvent",
            "is_native": false,
            "abilities": [
              "drop",
              "store"
            ],
            "generic_type_params": [],
            "fields": [
              {
                "name": "from_message",
                "type": "0x1::string::String"
              },
              {
                "name": "to_message",
                "type": "0x1::string::String"
              }
            ]
          },
          {
            "name": "MessageHolder",
            "is_native": false,
            "abilities": [
              "key"
            ],
            "generic_type_params": [],
            "fields": [
              {
                "name": "message",
                "type": "0x1::string::String"
              },
              {
                "name": "message_change_events",
                "type": "0x1::event::EventHandle<0xb9bd2cfa58ca29bce1d7add25fce5c62220604cd0236fe3f90d9de91ed9fb8cb::Message::MessageChangeEvent>"
              }
            ]
          }
        ]
      }
    }
  ]
}
```
