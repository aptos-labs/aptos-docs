...
Este es el primer capítulo del tutorial sobre [construir una dapp end-to-end en Aptos](/es/build/guides/build-e2e-dapp). Si aún no lo has hecho, revisa esa introducción y asegúrate de que tu entorno cumpla con los [prerrequisitos](/es/build/guides/build-e2e-dapp#prerequisites) listados allí.
...
<Aside type="note">
  Crear un evento no es obligatorio, pero es útil si las dapps o los usuarios quieren monitorear datos, como cuántas personas crean una nueva tarea, utilizando el [Aptos Indexer](/es/build/indexer).
</Aside>
...
Ahora vamos a [configurar el frontend](/es/build/guides/build-e2e-dapp/2-set-up-the-frontend) en el capítulo 2.
