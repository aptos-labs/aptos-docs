...
En el quinto y último capítulo del tutorial sobre [construir una dapp de extremo a extremo en Aptos](/es/build/guides/build-e2e-dapp), agregarás funcionalidad a la aplicación para que la interfaz de usuario pueda manejar casos donde una cuenta ha creado una lista.

Hemos cubierto cómo [obtener datos](/es/build/guides/build-e2e-dapp/3-fetch-data-from-chain) (una lista de tareas de cuenta) de la cadena y cómo [enviar una transacción](/es/build/guides/build-e2e-dapp/4-submit-data-to-chain) (nueva lista de tareas) a la cadena usando Billetera.
