...
[Instala el binario precompilado para el Aptos CLI](/es/build/cli).
...
<Tabs>
  ...
  <TabItem label="Python">
    ...
    A continuación, construye el paquete usando la CLI:
...
Los contratos Move son efectivamente un conjunto de módulos Move conocidos como un paquete. Al desplegar o actualizar un nuevo paquete, se debe invocar el compilador con `--save-metadata` para publicar el paquete. En el caso de MoonCoin, los siguientes archivos de salida son fundamentales:
...
Una vez que un tipo de moneda ha sido publicado en la blockchain de Aptos, la entidad que publicó ese tipo de moneda puede inicializarlo:
...
Para usar una moneda, una entidad debe registrar un `CoinStore` para ella en su cuenta:

```move
public entry fun registerCoinType(account: &signer) {
```

MoonCoin usa `ManagedCoin` que proporciona un envoltorio de función de entrada: `managed_coin::register`. Aquí hay un script de ejemplo para el registro:
...
- [TypeScript SDK](/es/build/sdks/ts-sdk)
- [Python SDK](/es/build/sdks/python-sdk)
- [REST API specification](/es/network/nodes/aptos-api-spec)
