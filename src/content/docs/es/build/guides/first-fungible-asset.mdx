...
## Entendiendo el contrato de ejemplo `fa_coin.move`

El contrato completo de FACoin.move se puede encontrar [aquí](https://github.com/aptos-labs/aptos-ts-sdk/blob/main/examples/typescript/move/facoin/sources/fa_coin.move).

Veamos paso a paso cómo está escrito este contrato.
...
<Steps>
  ...
  2. Importaciones
     ...
     El módulo FACoin utiliza varios módulos importantes:

     1. `fungible_asset` contiene la lógica para otorgar permisos para acuñar, transferir, quemar y crear tu FungibleAsset.
     2. `object` permite crear Objetos de Aptos.
     3. `primary_fungible_store` contiene la lógica para rastrear los saldos de cuenta del nuevo Activo Funciable.
     ...
     Estas importaciones se definen en el archivo `Move.toml` como dependencias de GitHub.

  3. init\_module

     Esta función se llama cuando el módulo se publica inicialmente para configurar los permisos y Objetos adecuados. Para FACoin, se utiliza para inicializar el Objeto de `MetaData` del activo (que contiene el nombre y el símbolo del activo), así como para obtener las referencias relevantes sobre cómo se usará nuestro activo fungible.

     El estándar `ManagedFungibleAsset` ayuda a llevar registro de qué permisos puede usar este Módulo.
  ...
  4. Funciones de vista
     ...
     Al crear tu propio activo fungible, puede ser útil añadir funciones de vista para cualquier dato que se necesite más adelante. En este caso, quisimos ver el nombre del activo para indicar qué activo se estaba intercambiando en nuestro escenario de ejemplo.
  ...
  5. Funciones de entrada
     ...
     Todo activo fungible tiene una interfaz similar (acuñar, transferir, quemar, congelar, descongelar, depositar y retirar). Aquí hay un ejemplo de una función de acuñación mínima, que acuña y transfiere los fondos al destinatario correspondiente:
...
Si quieres construir tu propio Activo Funciable, puedes usar [`fa_coin.move`](https://github.com/aptos-labs/aptos-ts-sdk/tree/main/examples/typescript/move/facoin) como punto de partida, o revisar otros ejemplos de código [aquí](https://learn.aptoslabs.com/code-examples).

En cualquier caso, el Estándar de Activo Funciable te ayudará a acuñar, transferir, quemar y llevar el control de balances automáticamente para los activos fungibles que quieras representar on-chain.

Puedes encontrar la [referencia de Move para Activos Fungibles](/move-reference/mainnet/aptos-framework/fungible_asset) para más detalles sobre las firmas de funciones y detalles de implementación.
