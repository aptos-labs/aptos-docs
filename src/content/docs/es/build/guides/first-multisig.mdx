...
description: "Crea y gestiona cuentas multisig que requieren múltiples aprobaciones para transacciones usando el SDK de Python con ejemplos prácticos."
sidebar:
  label: "Primera Multisig"
---
...
<Aside type="note">
  Si vienes de Ethereum/Solidity, nota que Aptos maneja las cuentas multisig de manera diferente. Aptos implementa [multisig directamente a nivel de protocolo](/es/network/blockchain/accounts), permitiendo que las cuentas requieran múltiples firmas sin desplegar contratos inteligentes adicionales.

  ![Diagrama de Multisig](~/images/multisig_chart.svg)

  Estamos interfaciando con Aptos usando el [SDK de Python de Aptos](/es/build/sdks/python-sdk).
</Aside>
...
<Steps>
  ...
  3. Ejecutar el script
     ...
     <Aside type="note">
       Para cada usuario, nota la [dirección de la cuenta](/es/network/blockchain/accounts#dirección-de-la-cuenta) y la [clave de autenticación](/es/network/blockchain/accounts#clave-de-autenticación) son idénticas, pero la [clave pública](/es/network/blockchain/accounts#crear-una-cuenta) es diferente.
...
2. Aprende sobre [gobierno multisig en este tutorial](/es/build/cli/working-with-move-contracts/multi-signature-tutorial).
3. Explora [abstracción de cuenta en Aptos](/es/network/blockchain/accounts).
