...
description: "Guía de integración heredada para integradores de sistemas - obsoleta en favor de la guía de integración de exchanges."
sidebar:
...
<Aside type="note">
  Esta guía está en proceso de ser reemplazada. Por favor revisa la [guía de integración de exchanges](/es/build/guides/exchanges)
...
consulta [SDKs](/es/build/sdks) para más información.

Casi todos los desarrolladores beneficiarán de explorar el CLI.
[Usando el CLI](/es/build/cli) demuestra cómo el CLI puede ser utilizado para crear
cuentas, transferir monedas, publicar módulos Move, y más.

## Cuentas en Aptos

Una [cuenta](/es/network/blockchain/accounts) representa
...
[módulos Move y recursos](/es/network/blockchain/resources).
...
- Un estrictamente
  creciente [número de secuencia](/es/network/blockchain/accounts#account-sequence-number)
...
- Un [manejador de eventos](/es/network/blockchain/events) para todos los nuevos tipos de monedas agregadas a
  la cuenta.
- Un manejador de eventos para todas las rotaciones de claves de la cuenta.

Lee más sobre [Cuentas](/es/network/blockchain/accounts)
y [configúrala](/es/build/cli/setup-cli).

## Transacciones

Las [transacciones](/es/network/blockchain/txns-states) de Aptos están codificadas
...
Lee más en [Transacciones y Estados](/es/network/blockchain/txns-states).
...
- A medida que las transacciones se ejecutan, pueden emitir eventos. [Eventos](/es/network/blockchain/events)
...
Para saber qué API llamar, por favor refiérase a la tabla [`concurrent_fungible_asset_balance`](/es/build/indexer/indexer-api/fungible-asset-balances) campo `standard`, donde "v1" significa usar API de moneda y "v2" significa usar API de activo fungible.

### Balance actual para Activo Fungible

Los usuarios de indexador pueden simplemente consultar [`concurrent_fungible_asset_balance`](/es/build/indexer/indexer-api/fungible-asset-balances) para obtener el balance.
