...
description: "Comprende la arquitectura del Indexador de Aptos: Servicio de Flujo de Transacciones, procesadores personalizados, integración de base de datos y estructura de API"
sidebar:
  label: "Arquitectura"
---
...
1. Datos históricos - Ej. [¿Qué transacciones han impactado esta cuenta?](/es/build/indexer/indexer-api/account-transactions)
2. Datos agregados - Ej. [¿Cuántos delegadores hay en este pool de staking?](/es/build/indexer/indexer-api/get-delegators)
3. Información específica mejor buscada vía consulta - Ej. [¿Qué NFTs posee una cuenta?](/es/build/indexer/indexer-api/get-nfts)
...
El Indexador usa el [Servicio de Stream de Transacciones](/es/build/indexer/txn-stream) y procesadores personalizados escritos con el [SDK del Indexador](/es/build/indexer/indexer-sdk) para actualizar una base de datos con tablas ricas. Luego expone una API para que las aplicaciones de Aptos accedan a los datos consolidados.

Para situaciones donde necesitas ir más allá de los datos de la API del Indexador alojada por Aptos, querrás crear un procesador personalizado con el [SDK del Indexador](/es/build/indexer/indexer-sdk).
...
Si te gustaría operar tu propia API del Indexador como servicio, consulta cómo [alojar tu propio Indexador](/es/build/indexer/indexer-api/self-hosted).
