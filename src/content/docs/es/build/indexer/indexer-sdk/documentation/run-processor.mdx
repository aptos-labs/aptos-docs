...
Por favor primero lee [Crear un Procesador](/es/build/indexer/indexer-sdk/documentation/create-processor), [Crear un Paso](/es/build/indexer/indexer-sdk/documentation/steps), y [Conectar Pasos](/es/build/indexer/indexer-sdk/documentation/connect-steps), que configurarán tu procesador y conectarán los pasos de tu procesador.
...
Para obtener el `indexer_grpc_data_service_address` y `auth_token`, puedes seguir la guía [aquí](/es/build/indexer/txn-stream/aptos-hosted-txn-stream).

`TransactionStreamConfig` también soporta más campos opcionales para modificar la conexión a [Transaction Stream](/es/build/indexer/txn-stream), sobre lo cual puedes aprender más [aquí](https://github.com/aptos-labs/aptos-indexer-processor-sdk/tree/main/aptos-indexer-processors-sdk/transaction-stream).
