...
Sigue [esta guía](/build/indexer/txn-stream/aptos-hosted-txn-stream#authorization-via-api-key) para obtener un token del Portal de Desarrolladores. Crea una API Key para `Testnet`, ya que este tutorial es para `Testnet`.
