...
<Steps>
  1. Construir
     ...
     1. **La cuenta `sender`.** <br />Esta cuenta normalmente paga las tarifas de gas por esta transacción. Ver [Patrocinio de Transacciones](/es/build/sdks/ts-sdk/building-transactions/sponsoring-transactions) para aprender cómo hacer que otra cuenta pague las tarifas.
 ...
 * ¡No olvides instalar @aptos-labs/ts-sdk antes de ejecutar este ejemplo!
 */
...
Construir y enviar transacciones on-chain implica los siguientes 5 pasos:
...
1. [Multi-Agent Transactions](/es/build/sdks/ts-sdk/building-transactions/multi-agent-transactions) - Allowing multiple accounts to interact with a single transaction.
2. [Orderless Transactions](/es/build/sdks/ts-sdk/building-transactions/orderless-transactions) - Allowing for transactions to be executed out of order for easier management.
3. [Sponsoring Transactions](/es/build/sdks/ts-sdk/building-transactions/sponsoring-transactions) - Have another account pay gas fees for this transaction.
4. [Batch Submit Transactions](/es/build/sdks/ts-sdk/building-transactions/batching-transactions) - How to send multiple transactions quickly from a single account.
5. [Binary Canonical Serialization (BCS)](/es/build/sdks/ts-sdk/building-transactions/bcs-format) - The format used to serialize data for Aptos transactions.
6. [Composing multiple Move calls with ScriptComposer](/es/build/sdks/ts-sdk/building-transactions/script-composer) - (Experimental) Building more complex transaction payload that calls into multiple Move functions dynamically.
