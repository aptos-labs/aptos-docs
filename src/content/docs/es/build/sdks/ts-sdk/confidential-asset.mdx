...
Para interactuar con el activo confidencial, primero crea un [par de claves único](/es/build/sdks/ts-sdk/confidential-asset#confidential-asset-store)
...
O usa [`pepper`](/es/build/guides/aptos-keyless/how-keyless-works) de [Cuenta Keyless](/es/build/guides/aptos-keyless)

### Registro

Luego, necesitas [registrar](/es/build/sdks/ts-sdk/confidential-asset#register) la clave de encriptación generada anteriormente (EK) en los contratos:
...
Este método te devuelve el `pending` y `actual` balance confidencial del usuario, y para [descifrar](/es/build/sdks/ts-sdk/confidential-asset#encryption-and-decryption)los, puedes usar la clase `ConfidentialAmount`
...
El usuario no puede operar con el balance `pending`, por lo que podrías [rollover](/es/build/sdks/ts-sdk/confidential-asset#rollover-pending-balance)lo a un `actual` balance.

Y para hacerlo, usa `aptos.confidentialAsset.rolloverPendingBalance`.

<Aside type="caution">
  Nota importante, que el balance `actual` del usuario necesita ser [normalizado](/es/build/sdks/ts-sdk/confidential-asset#normalize) antes de la operación de `rollover`.
</Aside>
...
Normalmente no necesitas llamar explícitamente a [normalización](/es/build/sdks/ts-sdk/confidential-asset#normalize)
...
Para [retirar](/es/build/sdks/ts-sdk/confidential-asset#withdraw) tus activos del balance confidencial:
...
Para [transferir](/es/build/sdks/ts-sdk/confidential-asset#confidential-transfer) necesitas conocer la clave de encriptación y la dirección de cuenta de `aptos`
...
Para hacer [rotación de clave](/es/build/sdks/ts-sdk/confidential-asset#rotate-encryption-key), necesitas crear una nueva clave de descifrado y usar `aptos.confidentialAsset.rotateCBKey`
