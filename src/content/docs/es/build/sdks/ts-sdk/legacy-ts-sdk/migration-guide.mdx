...
description: "Guía completa para migrar desde el SDK TypeScript legacy (v1.x.x) al nuevo paquete @aptos-labs/ts-sdk"
sidebar:
...
<Aside emoji="🚨" type="danger">
  El paquete SDK TypeScript Legacy `aptos` está deprecado y será reemplazado por el nuevo SDK TypeScript. Por favor consulta el [nuevo SDK TypeScript](/build/sdks/ts-sdk) para las últimas características y actualizaciones.
</Aside>

Si vienes de una versión anterior `1.x.x` de `aptos`, necesitarás hacer las siguientes actualizaciones.

<Aside type="note">
  Esta guía solo contiene diferencias de API y actualizaciones requeridas para características deprecadas. Las nuevas características del SDK v2 no están incluidas.
</Aside>

## Instalar el SDK

El SDK TypeScript V2 está bajo un nuevo [repositorio GitHub](https://github.com/aptos-labs/aptos-ts-sdk) y con un nuevo nombre de paquete - `@aptos-labs/ts-sdk`
...
<Aside type="note">
  Lee más sobre esto [aquí](/build/sdks/ts-sdk).
</Aside>
...
<Aside type="note">
  Lee más sobre esto [aquí](/build/sdks/ts-sdk/building-transactions).
</Aside>
...
<Aside type="note">
  Lee más sobre esto [aquí](/build/sdks/ts-sdk/account).
</Aside>
