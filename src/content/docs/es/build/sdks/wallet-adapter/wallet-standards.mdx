...
<Aside type="caution" title="Las APIs legacy de wallet están obsoletas">
  El acceso directo a wallets mediante variables globales del navegador (como `window.aptos` o `window.petra`) está **obsoleto** y no debe usarse. Estas APIs legacy ya no se mantienen y pueden ser eliminadas de las wallets en cualquier momento. Todas las dapps deben usar el [Adaptador de Wallet de Aptos](/es/build/sdks/wallet-adapter/dapp) con el estándar [AIP-62](https://github.com/aptos-foundation/AIPs/blob/main/aips/aip-62.md) para la integración de wallets.
</Aside>
