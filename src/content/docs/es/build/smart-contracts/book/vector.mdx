...
`vector` proporciona varias operaciones vía el módulo `std::vector` en la biblioteca estándar de Move, como se muestra abajo. Más operaciones pueden ser agregadas con el tiempo. Documentación actualizada sobre `vector` se puede encontrar [aquí](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/move-stdlib/vector).
