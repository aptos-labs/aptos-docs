...
description: "Aprende cómo crear paquetes Move para el desarrollo de contratos inteligentes en Aptos."
sidebar:
  label: "Crear Paquete"
---
...
<Aside type="note">
  ...
  Si aún no has instalado la CLI de Aptos, consulta la [sección CLI](/es/build/cli)
</Aside>
...
<Steps>
  1. aptos move init
     ...
     <Aside type="note">
       También puedes crear un paquete Move desde una [plantilla](/es/build/cli/start-from-template).
     </Aside>
  ...
  2. Actualizar Move.toml
     ...
     4. `dependencies`: Probablemente querrás usar `AptosFramework` y otras [Dependencias de Terceros](/es/build/smart-contracts/third-party-dependencies)
