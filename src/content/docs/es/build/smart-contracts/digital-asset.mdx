...
description: "Aprende sobre el estándar de activos digitales de Aptos para el desarrollo de contratos inteligentes Move."
sidebar:
...
Transferring a `Token` can be done by calling [`object::transfer`](/es/move-reference/mainnet/aptos-framework/object#transfer).
...
See the `aptos_token` [reference docs](/es/move-reference/mainnet/aptos-token-objects/aptos_token) for all the helper functions you can use.
...
To mint a soul bound `Token`, you can call [`aptos_token::mint_soul_bound`](/es/move-reference/mainnet/aptos-token-objects/aptos_token#mint_soul_bound) instead:
