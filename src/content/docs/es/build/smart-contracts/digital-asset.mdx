...
Este estándar reemplaza el legado [Estándar de Token de Aptos](/es/build/smart-contracts/aptos-token). Las mejoras más importantes a notar son:

| **Mejora**         | **Descripción**                                                                                                |
| ----------------------- | -------------------------------------------------------------------------------------------------------------- |
| **Extensión de Token**     | Los tokens pueden ser fácilmente extendidos ya que están implementados usando [Objetos](/es/build/smart-contracts/objects) de Move. |
...
Since each `Collection` is a [Move Object](/es/build/smart-contracts/objects), you can customize it by generating permissions called `Ref`s. Each `Ref` allows you to modify an aspect of the Object later on. Beyond the normal [Object Refs](/es/build/smart-contracts/object/creating-objects), `Collection`s can also get a `MutatorRef` by calling `get_mutator_ref` like so:
...
Puedes personalizar aún más tu `Collection` agregando más recursos o funcionalidades a través de un contrato inteligente. Por ejemplo, una `Collection` puede rastrear cuándo se creó para limitar cuándo se pueden acuñar `Token`s así:
...
| **Campo**       | **Descripción**                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                              |
| --------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **Descripción** | Una cadena opcional más pequeña que 2048 caracteres (modificable con un `MutatorRef`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                       |
| **Nombre**      | Una cadena requerida para identificar el `Token` que es única dentro de cada `Collection`. Esto significa que una sola cuenta de `Collection` no puede tener más de un `Token` con el mismo nombre.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                   |
| **Royalty**     | Una estructura opcional [`Royalty`](https://aptos.dev/reference/move/?branch=mainnet\&page=aptos-token-objects/doc/royalty.md#0x4_royalty_Royalty) que indica qué % del precio de venta va al creador de la `Collection`. Esto puede cambiarse con un `MutatorRef` generado por el [módulo Royalty](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-token-objects/sources/royalty.move) (una extensión para el estándar DA). Consulta el uso de ejemplo en [`aptos_token.move`](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-token-objects/sources/aptos_token.move)). Normalmente las regalías se establecen en las colecciones, pero configurarlas en los `Token`s permite que cada `Token` tenga un monto de regalía personalizado. |
| **Longitud de URI**  | Una cadena opcional más pequeña que 512 caracteres que enlaza a contenido relevante para el `Token` (modificable con un `MutatorRef`).                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                  |
...
Puedes encontrar las direcciones de tus `Token`s “sin nombre” recién creados usando el [Indexador de Aptos](/es/build/indexer/indexer-api) con consultas como las siguientes:
...
Transferir un `Token` puede hacerse llamando a [`object::transfer`](https://aptos.dev/reference/move/?branch=mainnet\&page=aptos-framework/doc/object.md#0x1_object_transfer).
...
<Aside type="caution">
  En un futuro cercano, se lanzará un nuevo módulo `TokenMinter` para reemplazar `aptos_token`. Puedes seguir el estado de esa propuesta [aquí](https://github.com/aptos-foundation/AIPs/blob/main/aips/aip-72.md).
</Aside>
