...
module my_addr::object_playground {
  ...
  entry fun create_my_object(
    ...
    // Ahora tenemos una opción, podemos hacer que el objeto pueda ser transferido
    // y podemos decidir si queremos permitir que cambie más tarde. Por defecto, esto
    // is transferrable
    if (!transferrable) {
  ...
  /// En este ejemplo, solo permitiremos que el publicador del contrato cambie los
  ...
  entry fun toggle_transfer(
    ...
    // Alternarlo basado en su estado actual
...
module my_addr::object_playground {
  ...
  /// En este ejemplo, solo permitiremos que el publicador del contrato cambie los
  /// permisos de transferencia
  /// Ahora solo el propietario puede transferir exactamente una vez
  entry fun transfer(
    caller: &signer,
    object: Object<ObjectController>,
    new_owner: address
...
module my_addr::object_playground {
  ...
  /// Ahora solo permitir que el propietario elimine el objeto
  entry fun delete(
    caller: &signer,
    object: Object<ObjectController>,
  ) acquires ObjectController {
    ...
    // Recuperar la delete ref, se consume por lo que debe ser extraída
    // from the resource
...
También puedes explorar cómo usar Objetos una vez que están construidos [aquí](/es/build/smart-contracts/object/using-objects).
