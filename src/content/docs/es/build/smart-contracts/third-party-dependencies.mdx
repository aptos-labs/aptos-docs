...
description: "Aprende cómo gestionar dependencias de terceros en proyectos Move para contratos inteligentes de Aptos."
sidebar:
  label: "Dependencies"
---
...
Las dependencias de terceros son [módulos](/es/build/smart-contracts/book/modules-and-scripts) externos con los que interactúa un módulo controlado. Típicamente, estos módulos externos existen bajo diferentes cuentas.

## Ejemplo de router multi-DEX

Un router multi-DEX utiliza activamente dependencias de terceros. En lugar de enviar múltiples transacciones para interactuar con diferentes DEXs y sus funciones de [entrada](/es/build/smart-contracts/book/functions#entry-modifier) individuales dentro de una ruta de intercambio, un módulo (o [script](/es/build/smart-contracts/book/modules-and-scripts#scripts)) puede consolidar todas las invocaciones independientes de DEX en una sola transacción atómica. El router multi-DEX referencia y llama a funciones presentes en cada uno de los módulos DEX de terceros para lograr esto.
...
El Aptos CLI permite descargar el bytecode de un [paquete](/es/build/smart-contracts/book/packages).
