...
- [Nodos validator (VNs)](/es/network/blockchain/validator-nodes) - participan en consenso y impulsan el [procesamiento de transacciones](/es/network/blockchain/txns-states).
...
- [Fullnodes públicos (PFNs)](/es/network/blockchain/fullnodes) - ejecutados por alguien que no es operador de validator, los PFNs no pueden conectarse directamente a un nodo validator y por lo tanto dependen de VFNs para sincronización.
- [Nodos archivales (ANs)](/es/network/nodes/configure/state-sync#archival-pfns) - es un fullnode que contiene todos los datos de blockchain desde el inicio del historial de la blockchain.
