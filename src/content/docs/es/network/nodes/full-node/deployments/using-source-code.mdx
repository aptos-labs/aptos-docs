...
Para desplegar un PFN usando el código fuente `aptos-core`, primero, ver [Construir Aptos Desde Código Fuente](/es/network/nodes/building-from-source) para instrucciones
...
3. Siguiente, descarga los archivos `genesis.blob` y `waypoint.txt` para la red a la que tu PFN se conectará:
   ...
   <Aside type="caution">
     **¿No quieres conectarte a mainnet?**<br />
     Para conectarte a otras redes (ej., `devnet` y `testnet`), puedes encontrar el génesis y waypoint aquí ➜ [https://github.com/aptos-labs/aptos-networks](https://github.com/aptos-labs/aptos-networks).
