...
Primero, usa el [CLI de Aptos](/build/cli) (`aptos`) para producir una clave privada estática
...
- **Usando la API REST**: Puedes consultar el endpoint `/v1/info` de la [API REST](/build/apis/fullnode-rest-api) de tu nodo
...
- **Usando el Servicio de Inspección de Nodos**: Puedes consultar el endpoint `/identity_information` en el servicio de inspección de tu nodo
  para recuperar la información de identidad de tu nodo. Puedes leer más [aquí](/network/nodes/measure/node-inspection-service#examine-identity-information).
