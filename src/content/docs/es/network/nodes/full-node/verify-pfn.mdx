...
<Aside type="note">
  Puedes aprender más sobre las métricas de Aptos en los documentos del [Servicio de Inspección de Nodos](/es/network/nodes/measure/node-inspection-service).
</Aside>
...
en la guía de [sincronización de estado](/es/network/nodes/configure/state-sync)). Puedes monitorear el progreso de sincronización de estado
...
<Aside type="caution">
  **¿Usando sincronización rápida?**<br />
  ...
  Puedes leer más sobre sincronización rápida aquí: [Inicialización de sincronización de estado](/es/network/nodes/configure/state-sync#bootstrapping-phase).
</Aside>
...
1. Actualiza tu nodo a la última versión siguiendo [Actualizar tu Nodo](/es/network/nodes/full-node/modify/update-fullnode-with-new-releases).
2. Remueve cualquier par `seed` que puedas haber agregado a tu archivo de configuración. Las semillas pueden estar impidiendo que te
   conectes a la red. Los pares semilla se discuten en la sección [Conectar tu PFN a pares semilla](/es/network/nodes/full-node/modify/fullnode-network-connections#connecting-to-seed-peers).
