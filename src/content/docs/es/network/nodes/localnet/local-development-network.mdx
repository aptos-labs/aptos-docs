...
<Steps>
  1. Asegúrate de tener el [CLI de Aptos](../../../build/cli) instalado.
  ...
  5. Espera a que la red inicie
     ...
     - [API del Nodo](/network/nodes/aptos-api-spec): Esta es una API REST que se ejecuta directamente en el nodo. Habilita funcionalidad central de escritura como envío de transacciones y un conjunto limitado de funcionalidad de lectura, como leer recursos de cuenta o información de módulos Move.
     - [API del Indexer](/build/indexer/indexer-api): Esta es una API [GraphQL](https://graphql.org/) que proporciona acceso de lectura rico a datos de blockchain indexados. Si haces clic en la URL para la API del Indexer arriba, por defecto [http://127.0.0.1:8090](http://127.0.0.1:8090/), abrirá la Consola Hasura, una UI web que te ayudará a consultar la API GraphQL del Indexer.
     - [Servicio de Stream de Transacciones](/build/indexer/txn-stream): Este es un stream gRPC de transacciones usado por la API del Indexer. Esto solo es relevante para ti si estás desarrollando un procesador personalizado del [SDK del Indexer](/build/indexer/indexer-sdk).
     ...
     - [Faucet](/build/apis/faucet-api): Puedes usar esto para fondear cuentas en tu red local.
...
- Estás construyendo un [procesador de indexer personalizado](/build/indexer) y te gustaría indexar usando una red fresca.
