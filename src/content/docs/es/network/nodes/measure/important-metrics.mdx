...
Cuando visites el endpoint de métricas (ver [Servicio de Inspección de Nodos](/network/nodes/measure/node-inspection-service)), notarás que
...
métricas de [consenso](/network/blockchain/blockchain-deep-dive#consensus) son importantes:
...
), las siguientes métricas de [sincronización de estado](/network/nodes/configure/state-sync) son importantes:
...
1. `aptos_state_sync_version{type="synced"}`: Esta métrica muestra la versión sincronizada actual del nodo,
   ...
   la red. Nota: si has seleccionado usar [sincronización rápida](/network/nodes/configure/state-sync#fast-syncing),
...
3. `aptos_state_sync_version{type="synced_states"}`: Esta métrica cuenta el número de estados que han sido
   descargados mientras un nodo está [sincronizando rápido](/network/nodes/configure/state-sync#fast-syncing). Si esta métrica no aumenta,
...
1. `aptos_connections{direction="inbound"` y `aptos_connections{direction="outbound"`: Estas métricas cuentan
   ...
   2. Si tu nodo es un fullnode, el número de conexiones `outbound` debería ser `> 0`. Esto asegurará que tu nodo pueda
      ...
      [Conexiones de Red Fullnode](/network/nodes/full-node/modify/fullnode-network-connections#allowing-pfn-connections).

### Mempool

Las siguientes métricas de [mempool](/network/blockchain/blockchain-deep-dive#mempool) son importantes:
...
Las siguientes métricas de [API REST](/build/apis) son importantes: {/* TODO Maybe replace with API spec */}
