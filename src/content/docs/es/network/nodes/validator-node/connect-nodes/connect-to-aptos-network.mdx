...
<Aside type="caution">
  **REQUISITOS MÍNIMOS DE STAKING**<br />
  ...
  [requisitos mínimos de staking](/network/blockchain/staking#minimum-and-maximum-stake) para la
...
pools en las secciones [Staking](/network/blockchain/staking) y [Staking Delegado](/network/blockchain/delegated-staking).

Para inicializar un pool de staking, sigue las instrucciones en
[operaciones de pool de staking](/network/nodes/validator-node/connect-nodes/staking-pool-operations#initialize-a-staking-pool). De lo contrario,
para inicializar un pool de delegación, sigue las instrucciones en
[operaciones de pool de delegación](/network/nodes/validator-node/connect-nodes/delegation-pool-operations#initialize-a-delegation-pool).
...
6. Descarga los archivos `genesis.blob` y `waypoint.txt` para tu red. Ver [Archivos de Nodo](/network/nodes/configure/node-files-all-networks) para ubicaciones y comandos para descargar estos archivos.
...
<Aside type="caution">
  **MAINNET VS TESTNET**<br />
  ...
  valores en [Redes Blockchain de Aptos](/network/nodes/networks) para ver cómo los perfiles pueden configurarse basados en la red.
</Aside>
...
1. Inicia el VFN. El VFN se conectará a la red y comenzará a sincronizar. Ver [Sincronización de Estado](/network/nodes/configure/state-sync) para más información.
...
correctamente, visita el documento [Salud del Nodo](/network/nodes/validator-node/verify-nodes/node-liveness-criteria). Este documento describe cómo puedes verificar y
