...
1. [Instalar y configurar](/build/cli) el CLI de Aptos. Si estás buscando desarrollar en la blockchain de Aptos, debuggear apps, o realizar operaciones de nodo, la herramienta Aptos ofrece una interfaz de línea de comandos para estos propósitos.
2. [Inicializar configuración local y crear una cuenta](/build/cli/setup-cli) en la blockchain de Aptos.
...
5. El pool de delegación ahora puede [conectarse a la Red de Aptos](/network/nodes/validator-node/connect-nodes/connect-to-aptos-network).
...
Hasta que el pool de delegación haya recibido 1 millón APT y el validator haya sido agregado al conjunto de validators activos, no habrá recompensas que rastrear durante cada ciclo. Para obtener información sobre un pool de delegación, usa la [función View](/build/apis/fullnode-rest-api#reading-state-with-the-view-function) de Aptos
...
En el [SDK TypeScript de Aptos](/build/sdks/ts-sdk), una solicitud de función View se parecería a:
