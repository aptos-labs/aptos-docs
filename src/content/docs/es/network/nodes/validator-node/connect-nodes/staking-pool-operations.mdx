...
Este documento describe cómo realizar operaciones del pool de [staking](/network/blockchain/staking). Nota que un pool de staking solo puede aceptar stake
...
[Operaciones del Pool de Delegación](/network/nodes/validator-node/connect-nodes/delegation-pool-operations) para aceptar stake de múltiples delegadores para alcanzar el requisito mínimo
de staking.
...
<Aside type="note">
  **TESTNET VS MAINNET**<br />
  ...
  Ver los valores en [Redes Blockchain de Aptos](/network/nodes/networks) para ver cómo los perfiles pueden configurarse basados en la red.
</Aside>

Antes de inicializar un pool de staking, asegúrate de que haya una cuenta propietaria existente con 1 Millón APT.

1. Inicializa el [CLI de Aptos](/build/cli) con una clave privada de una cuenta existente, como una billetera, o crea una nueva cuenta.
...
3. Una vez que el pool de staking haya sido inicializado, puedes proceder a [conectarte a la Red de Aptos](/network/nodes/validator-node/connect-nodes/connect-to-aptos-network).
...
<Aside type="note">
  Antes de proceder, ver [Validación en la blockchain de Aptos](/network/blockchain/staking#validation-on-the-aptos-blockchain) para un breve resumen.
</Aside>
...
<Aside type="note">
  Para valores de campo `--url` de testnet o devnet, ver [Redes Blockchain de Aptos](/network/nodes/networks).
</Aside>
...
- Propuestas exitosas de líder-validator durante la época actual. También ver [Validación en la blockchain de Aptos](/network/blockchain/staking#validation-on-the-aptos-blockchain) para la distinción entre líder-validator y el voter-validator.
...
`DistributeEvent` o usando la [función View](/build/apis/fullnode-rest-api#reading-state-with-the-view-function)
