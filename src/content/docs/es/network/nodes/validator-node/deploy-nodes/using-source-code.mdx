...
<Aside type="caution">
  **Prerrequisitos**<br />
  ...
  instalado el [CLI de Aptos](/es/build/cli), ya que lo necesitarás para configurar tus nodos.
</Aside>
...
1. Sigue los pasos en [Construir Aptos Desde Código Fuente](/es/network/nodes/building-from-source) para descargar el repositorio `aptos-core` y código fuente.
...
6. Descarga los siguientes archivos siguiendo las instrucciones en las páginas [Archivos del Nodo](/es/network/nodes/configure/node-files-all-networks).
...
Ahora has completado la configuración de tu validator y VFN usando código fuente. Procede a [Conectar Nodos](/es/network/nodes/validator-node/connect-nodes)
