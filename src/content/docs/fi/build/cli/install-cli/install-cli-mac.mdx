...
<Aside type="caution">
  If `brew` does not work for you, you can try the steps here: [Asenna skriptillä](#install-via-script) or [Asenna valmiiksi käännetyillä binaareilla](#install-via-pre-compiled-binaries-backup-method).)
</Aside>
...
<Steps>
  1. Käytä päätteessä yhtä seuraavista komennoista:
...
# Asenna valmiiksi käännetyillä binaareilla (Backup Method)
