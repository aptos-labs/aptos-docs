...
<Aside type="caution">
  If `brew` does not work for you, you can try the steps here: [Instal via Script](#install-via-script) or [Instal via Biner yang Telah Dikompilasi](#install-via-pre-compiled-binaries-backup-method).)
</Aside>
...
<Steps>
  1. Di terminal, gunakan salah satu perintah berikut:
...
# Instal via Biner yang Telah Dikompilasi (Backup Method)
