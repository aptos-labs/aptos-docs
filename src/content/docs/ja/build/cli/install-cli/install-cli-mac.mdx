...
<Aside type="caution">
  If `brew` does not work for you, you can try the steps here: [スクリプトでインストール](#install-via-script) or [事前コンパイルされたバイナリでインストール](#install-via-pre-compiled-binaries-backup-method).)
</Aside>
...
# 事前コンパイルされたバイナリでインストール (Backup Method)
