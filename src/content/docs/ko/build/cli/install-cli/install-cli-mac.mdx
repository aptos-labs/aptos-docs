...
<Aside type="caution">
  If `brew` does not work for you, you can try the steps here: [스크립트로 설치](#install-via-script) or [사전 컴파일된 바이너리로 설치](#install-via-pre-compiled-binaries-backup-method).)
</Aside>
...
# 사전 컴파일된 바이너리로 설치 (Backup Method)
