...
description: "Deep dive into Aptos gas computation including instruction gas, storage fees, payload costs, and optimization strategies for cost-effective transactions."
sidebar:
  label: "Computing Gas"
---
...
Numeric parameter values in this page are **illustrative**. The live schedule is on-chain at `0x1::gas_schedule::GasScheduleV2` and in [`transaction.rs`](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/aptos-gas-schedule/src/gas_schedule/transaction.rs); governance can change it. Convert internal gas units to external gas units by dividing by `gas_unit_scaling_factor`, then to [octas](/network/glossary#octa) by multiplying by the gas unit price.
