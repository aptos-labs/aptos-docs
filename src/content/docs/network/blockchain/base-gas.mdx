...
The following storage fee parameters are applied (i.e., charged in absolute APT values) to represent the disk space and structural costs associated with using the [Aptos authenticated data structure](../../../network/glossary.mdx#merkle-trees) for storing items on the blockchain.
...
Then, to convert from external gas units to [octas](../../../network/glossary.mdx#octa), multiply by the "gas price", which denotes the number of octas per unit of external gas.
...
| Operation                    | Minimum [Octas](../../../network/glossary.mdx#octa) |
| ---------------------------- | ------------------------------------------- |
...
For comparison, reading a 100-byte item costs $r\_i + 100 \* r\_b = 3000 + 100 \* 3 = 3300$ [octas](../../../network/glossary.mdx#octa) at minimum, some 16.5 times as much as a function call, and in general, instruction gas costs are largely dominated by storage gas costs.
