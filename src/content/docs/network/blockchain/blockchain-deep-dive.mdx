...
- Alice and Bob are two users who each have an [account](/network/glossary.mdx#account) on the Aptos blockchain.
- Alice's account has 110 Aptos Coins.
- Alice is sending 10 Aptos Coins to Bob.
- The current [sequence number](/network/glossary.mdx#sequence-number) of Alice's account is 5 (which indicates that 5 transactions have already been sent from Alice's account).
- There are a total of 100 validator nodes — V<sub>1</sub> to V<sub>100</sub> on the network.
...
Client -> Fullnodes.REST_Service: "1"
Fullnodes.REST_Service -> Validators.Mempool: "2"
Validators.Mempool -> Other_Validators: "4"
Other_Validators -> Validators.Consensus: "6, 10"
Validators.Consensus -> Validators.Execution: "7, 9"
Validators.Execution -> Validators.Virtual_Machine: "8"
Validators.Execution -> Validators.Storage: "11"
Validators.Consensus -> Validators.Mempool: "5"
Validators.Virtual_Machine -> Validators.Storage
...
| Fields                                                                               | Description                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                            |
| ------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| [Account address](/network/glossary.mdx#account-address)                             | Alice's account address                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                           |
...
| [Maximum gas amount](/network/glossary.mdx#maximum-gas-amount)                       | The maximum gas amount in APT Alice is willing to pay for this transaction. Gas charges are equal to the base gas cost covered by computation and IO multiplied by the gas price. Gas costs also include storage with an Apt-fixed priced storage model. This is represented in [Octas](/network/glossary.mdx#octa).                                                                                                                                                                                                                                                                                   |
| [Expiration time](/network/glossary.mdx#expiration-time)                             | Expiration time of the transaction.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                    |
| [Sequence number](/network/glossary.mdx#sequence-number)                             | The sequence number (5, in this example) for an account indicates the number of transactions that have been submitted and committed on-chain from that account. In this case, 5 transactions have been submitted from Alice’s account, including Traw<sub>5</sub>. Note: a transaction with sequence number 5 can only be committed on-chain if the account sequence number is 5.                                                                                                                                                                                                                      |
...
| Description                                                                                                                                                                                                                                            | Aptos Node Component Interactions                                          |
| ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ | -------------------------------------------------------------------------- |
| 6. **Consensus → Mempool**: — As validator V<sub>1</sub> is a proposer/leader for this transaction, it will pull a block of transactions from its mempool and replicate this block as a proposal to other validator nodes via its consensus component. | [1. Consensus](#1-consensus--mempool), [3. Mempool](#3-consensus--mempool) |
...
| 10. **Consensus → Execution**: After executing the transactions in the block, the execution component appends the transactions in the block (including T<sub>5</sub>) to the [Merkle accumulator](/network/glossary.mdx#merkle-accumulator) (of the ledger history). This is an in-memory/temporary version of the Merkle accumulator. The necessary part of the proposed/speculative result of executing these transactions is returned to the consensus component to agree on. The arrow from "consensus" to "execution" indicates that the request to execute transactions was made by the consensus component. | [3. Consensus](#3-consensus--execution-consensus--other-validators), [1. Execution](#1-consensus--execution) |
...
Client -> REST_Service: "1"
REST_Service -> Mempool: "2"
REST_Service -> Storage: "3"
...
Mempool -> Virtual_Machine: "3"
Virtual_Machine -> Execution: "2"
Virtual_Machine -> Storage: "1"
...
Fullnodes.REST_Service -> Fullnodes.Mempool: "1"
Fullnodes.Mempool -> Other_Validators: "2"
Fullnodes.Mempool -> Fullnodes.Consensus: "3"
Fullnodes.Mempool -> Fullnodes.Virtual_Machine: "4"
...
Other_Validators <-> Validators.Consensus: "2, 4"
Validators.Consensus -> Validators.Mempool: "1"
Validators.Consensus -> Validators.Execution: "3, 4"
```

The consensus component is responsible for ordering blocks of transactions and agreeing on the results of execution by participating in the [consensus protocol](/network/glossary.mdx#consensus-protocol) with other validators in the network.
...
Consensus -> Execution: "1 and 3"
Execution -> Virtual_Machine: "2"
Execution -> Storage: "4"
...
- Execution maintains a "scratchpad," which holds in-memory copies of the relevant portions of the [Merkle accumulator](/network/glossary.mdx#merkle-accumulator). This information is used to calculate the root hash of the current state of the Aptos blockchain.
...
Virtual_Machine -> Storage: "1"
Execution -> Storage: "2, 3"
REST_Service -> Storage: "4"
```
...
Refer to [Merkle accumulator](/network/glossary.mdx#merkle-accumulator) for information on how a transaction is appended to the data structure representing the Aptos blockchain.
