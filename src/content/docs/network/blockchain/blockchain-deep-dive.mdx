...
description: "Take a comprehensive deep dive into Aptos blockchain architecture, transaction lifecycle, node components, and inter-component interactions for developers."
sidebar:
  label: "Blockchain Deep Dive"
---
...
- The current [sequence number](/network/glossary#sequence-number) of Alice's account is 5 (which indicates that 5 sequenced transactions have already been sent from Alice's account). This example uses a sequenced transaction. [Orderless transactions](/build/guides/orderless-transactions) use a nonce instead.
- There are many validator nodes on the network (the live count is on [Explorer](https://explorer.aptoslabs.com/validators?network=mainnet)).
...
graph LR
    ...
    QuorumStore <-->|4| Other_Validators(Other Validators)
    Consensus -->|5| Mempool
```
...
| Payload                                                                              | Indicates an action or set of actions on Alice's behalf. Typically this is a Move entry function with inputs; it can also be a [transaction script](/network/glossary#transaction-script). In this example, Alice transfers APT to Bob.                                                                          |
...
| [Sequence number](/network/glossary#sequence-number)                                 | For this sequenced example, 5. A sequenced transaction with sequence number 5 can commit only if the account sequence number is 5. [Orderless transactions](/build/guides/orderless-transactions) use a nonce instead and set this field to `u64::MAX`.                                                          |
...
| 3. **Mempool → Virtual Machine (VM)**: Mempool will use the virtual machine (VM) component to perform transaction validation, such as signature verification, account balance verification, and replay resistance using the sequence number or an orderless nonce.                                                                                                                                                                                            | [4. Mempool](#4-mempool--vm), [3. Virtual Machine](#3-mempool--virtual-machine)     |
...
| 5. **Quorum Store → Other Validators**: [Quorum Store](/network/glossary#quorum-store) batches transactions (including T<sub>5</sub>) and disseminates those batches to the validator set so data is available before the leader proposes a block. Shared mempool still moves transactions from fullnodes toward validators. | [2. Mempool](#2-mempool--other-validator-nodes) |

### Proposing the block

| Description                                                                                                                                                                                                                                          | Aptos Node Component Interactions                                          |
| ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------- |
| 6. **Consensus → Mempool / Quorum Store**: — As validator V<sub>1</sub> is a proposer/leader for this transaction, it pulls certified batches and forms a block, then replicates this proposal to other validator nodes via its consensus component. | [1. Consensus](#1-consensus--mempool), [3. Mempool](#3-consensus--mempool) |
...
| 10. **Consensus → Execution**: After executing the transactions in the block, the execution component appends the transactions in the block (including T<sub>5</sub>) to the [Merkle accumulator](/network/glossary#merkle-accumulator) (of the ledger history). This is an in-memory/temporary version of the Merkle accumulator. The necessary part of the proposed/speculative result of executing these transactions is returned to the consensus component to agree on. The arrow from "consensus" to "execution" indicates that the request to execute transactions was made by the consensus component. | [3. Consensus](#3-consensus--execution-consensus--other-validators), [1. Execution](#1-consensus--execution) |
...
For implementation details refer to the [Move VM in aptos-core](https://github.com/aptos-labs/aptos-core/tree/main/third_party/move/move-vm).
...
Mempool is a shared buffer that holds the transactions that are "waiting" to be executed. After a transaction is admitted, [Quorum Store](/network/glossary#quorum-store) disseminates it in batches to other validators so the leader's proposal can reference certified data instead of carrying every raw transaction. Shared mempool still forwards user transactions from fullnodes toward the validator set.
...
Once consensus is reached on a block of transactions, execution calls storage via `DbWriter::save_transactions()` to save the block of transactions and permanently record them. This will also store the signatures from the validator nodes that agreed on this block of transactions. The in-memory data in "scratchpad" for this block is passed to update storage and persist the transactions. When the storage is updated, every account that sent a **sequenced** transaction in the block has its sequence number incremented by one. [Orderless transactions](/build/guides/orderless-transactions) do not change the sender's sequence number.

Note: For sequenced transactions, the sender's sequence number increments by one for each committed transaction from that account. Orderless transactions use a nonce and leave the sequence number unchanged.
