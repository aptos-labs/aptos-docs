...
An Aptos node is an entity of the Aptos ecosystem that tracks the [state](../../../network/glossary.mdx#state) of the Aptos blockchain. Clients interact with the blockchain via Aptos nodes. There are two types of nodes:
...
- [REST API service](../../../network/glossary.mdx#rest-api-service)
- [Mempool](../../../network/blockchain/validator-nodes.mdx#mempool)
- [Execution](../../../network/blockchain/validator-nodes.mdx#execution)
- [Virtual Machine](../../../network/blockchain/validator-nodes.mdx#virtual-machine)
- [Storage](../../../network/blockchain/validator-nodes.mdx#storage)
- [State synchronizer](../../../network/blockchain/validator-nodes.mdx#state-synchronizer)

The [Aptos-core](../../../network/glossary.mdx#aptos-core) software can be configured to run as a validator node or as a fullnode.

## Overview

Fullnodes can be run by anyone. Fullnodes verify blockchain history by either re-executing all transactions in the history of the Aptos blockchain or replaying each transaction's output. Fullnodes replicate the entire state of the blockchain by synchronizing with upstream participants, e.g., other fullnodes or validator nodes. To verify blockchain state, fullnodes receive the set of transactions and the [accumulator root hash](../../../network/glossary.mdx#accumulator-root-hash) of the ledger signed by the validators. In addition, fullnodes accept transactions submitted by Aptos clients and forward them directly (or indirectly) to validator nodes. While fullnodes and validators share the same code, fullnodes do not participate in consensus.
