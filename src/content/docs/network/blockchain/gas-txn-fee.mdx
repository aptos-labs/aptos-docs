...
description: "Learn about gas fees and storage costs on Aptos, including execution costs, storage fees, gas pricing, and optimization strategies for efficient transactions."
sidebar:
...
Any transaction execution on the Aptos blockchain requires a processing fee. That fee has two components:
...
<Aside type="note">
  A **unit of gas** is a dimensionless integer. It is not a coin. The total gas units a transaction consumes depend on its complexity. **Gas unit price** is denominated in [Octas](/network/glossary#octa), the subunit of [APT](/network/glossary#apt). See [Transactions and States](/network/blockchain/txns-states) for the fields on a submitted transaction.
</Aside>
...
However, within a block, the order of transaction execution is determined by the system. This order is based on [transaction shuffling](https://github.com/aptos-foundation/AIPs/blob/main/aips/aip-027-sender-aware-transaction-shuffling.md), which makes parallel execution more efficient by considering conflict patterns. While in most cases this is unnecessary, if the network is under load this measure can ensure your transaction is processed more quickly. See the `gas_unit_price` entry under [Estimating the gas units via simulation](#estimating-gas-consumption-via-simulation) for details.

<Aside type="caution">
  If you raise `gas_unit_price` while the same account still has in-flight sequenced transactions, resubmit those
  earlier transactions at the higher price too. Sequenced transactions from one account must commit in sequence-number
  order, so a high-price transaction waits for the lower-price ones ahead of it. [Orderless transactions](/build/guides/orderless-transactions)
...
- `gas_unit_price`: The price per gas unit the transaction sender is willing to pay. It is expressed in [Octas](/network/glossary#octa).
  ...
  The transaction fee charged to the client will be at the most `gas_unit_price * max_gas_amount`.
...
There are some nuances with regard to price changing and legacy slots which didn't pay for the size of the slot below a historical "free quota". Refer to [AIP-65](https://github.com/aptos-foundation/AIPs/blob/main/aips/aip-065-storage-fee-for-state-bytes-refundable.md#specification) for more details.
...
To simulate transactions on chain, use the [`SimulateTransaction`](https://api.devnet.aptoslabs.com/v1/spec#/operations/simulate_transaction) API. This API runs the exact transaction you plan to submit.
...
<Aside type="note">
  Prioritization is based upon buckets of `gas_unit_price`. The buckets are `DEFAULT_BUCKETS` in [`global-constants`](https://github.com/aptos-labs/aptos-core/blob/main/config/global-constants/src/lib.rs), used as `broadcast_buckets` in [`mempool_config.rs`](https://github.com/aptos-labs/aptos-core/blob/main/config/src/config/mempool_config.rs). The current buckets are `[0, 150, 300, 500, 1000, 3000, 5000, 10000, 100000, 1000000]`. Therefore, a `gas_unit_price` of 150 and 299 would be prioritized nearly the same.
</Aside>
