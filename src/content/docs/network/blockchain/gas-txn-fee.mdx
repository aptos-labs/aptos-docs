...
- `gas_price`: The price per gas unit the transaction sender is willing to pay. It is expressed in [Octas](../../../network/glossary.mdx#octa).
...
Here is an example. Suppose we have a transaction that costs `100` gas units in execution & IO, and `5000` [Octas](../../../network/glossary.mdx#octa) in storage fees. The network will show that you have used
