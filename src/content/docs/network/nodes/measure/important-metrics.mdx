...
[consensus](../../../../network/blockchain/blockchain-deep-dive.mdx#consensus) metrics are important:
...
1. `aptos_state_sync_version{type="synced"}`: This metric displays the current synced version of the node,
   ...
   the network. Note: if you've selected to use [fast sync](../../../../network/nodes/configure/state-sync.mdx#fast-syncing),
...
3. `aptos_state_sync_version{type="synced_states"}`: This metric counts the number of states that have been
   downloaded while a node is [fast syncing](../../../../network/nodes/configure/state-sync.mdx#fast-syncing). If this metric doesn't increase,
...
1. `aptos_connections{direction="inbound"` and `aptos_connections{direction="outbound"`: These metrics count
   ...
   2. If your node is a fullnode, the number of `outbound` connections should be `> 0`. This will ensure your node is
      ...
      [Fullnode Network Connections](../../../../network/nodes/full-node/modify/fullnode-network-connections.mdx#allowing-pfn-connections).

### Mempool

The following [mempool](../../../../network/blockchain/blockchain-deep-dive.mdx#mempool) metrics are important:
