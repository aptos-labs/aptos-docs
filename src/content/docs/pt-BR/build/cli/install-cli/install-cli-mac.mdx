...
<Aside type="caution">
  If `brew` does not work for you, you can try the steps here: [Instalar via Script](#install-via-script) or [Instalar via Binários Pré-Compilados](#install-via-pre-compiled-binaries-backup-method).)
</Aside>
...
<Steps>
  1. No terminal, use um dos seguintes comandos:
...
# Instalar via Binários Pré-Compilados (Backup Method)
