...
description: "Complete installation guide for the Aptos CLI on macOS using Homebrew, shell scripts, or pre-compiled binaries with upgrade instructions."
sidebar:
  label: "Mac Installation"
---
...
<Aside type="caution">
  If `brew` does not work for you, you can try the steps here: [Установка через скрипт](#install-via-script) or [Установка через предварительно скомпилированные бинарные файлы](#install-via-pre-compiled-binaries-backup-method).)
</Aside>
...
<Steps>
  1. В терминале используйте одну из следующих команд:
...
# Установка через предварительно скомпилированные бинарные файлы (Backup Method)
