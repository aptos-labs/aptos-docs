---
title: "Aptos全节点REST API"
---

import { Aside, CardGrid, LinkCard } from '@astrojs/starlight/components';

该API嵌入到全节点中,提供了一种简单,低延迟,但低级别的方式来读取状态和提交交易到Aptos区块链.它还支持交易模拟.
对于更高级的查询,我们建议使用 [Indexer GraphQL API](/zh/build/indexer).

## Aptos全节点REST API浏览器

<CardGrid>
  <LinkCard href="https://fullnode.mainnet.aptoslabs.com/v1/spec#/" title="主网全节点 REST API" description="主网的REST API浏览器" target="_blank" />

<LinkCard href="https://fullnode.testnet.aptoslabs.com/v1/spec#/" title="测试网全节点 REST API" description="测试网的REST API浏览器" target="_blank" />

<LinkCard href="https://fullnode.devnet.aptoslabs.com/v1/spec#/" title="开发网全节点 REST API" description="开发网的REST API浏览器" target="_blank" />
</CardGrid>

## 理解速率限制

与 [Aptos Indexer](/zh/build/indexer/indexer-api) 一样,Aptos REST API对每个IP地址的请求有一个每五分钟 5000 个的速率限制,无论是提交交易还是查询Aptos提供的节点上的API.(作为节点运营商,您可以在自己的节点上提高这些限制.)请注意,此限制可能会在有或没有事先通知的情况下更改.
...
- 当交易执行时,它们可能会发出事件.[事件](/zh/network/blockchain/events)是关于链上数据变化的提示.

<Aside type="note">
  确保您正在与之通信的 [全节点](/zh/network/nodes/networks)是最新的.
  全节点必须达到包含您的交易的版本,才能从中检索相关数据.
  全节点从[验证者全节点](/zh/network/blockchain/fullnodes)检索状态可能会有延迟,而验证者全节点又依赖于[验证者节点](/zh/network/blockchain/validator-nodes)作为可信来源.
</Aside>
...
要使用 View 函数,需要通过 [Aptos CLI](/zh/build/cli) 将模块发布.
