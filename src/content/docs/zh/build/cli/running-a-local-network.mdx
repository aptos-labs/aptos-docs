...
description: "使用 Docker 设置和运行本地 Aptos 网络进行测试和开发，包括 Node API、索引器 API 和水龙头服务。"
sidebar:
  label: "运行本地网络"
---
...
<Steps>
  ...
  5. 等待网络启动
     ...
     - [节点 API](/zh/network/nodes/aptos-api-spec):这是直接在节点上运行的 REST API.它支持核心写功能(如交易提交)和有限的读功能,如读取账户资源或 Move 模块信息.
     - [索引器 API](/zh/build/indexer/aptos-hosted):这是一个 [GraphQL](https://graphql.org/) API,提供对索引区块链数据的丰富读取访问.如果你点击上面的索引器 API URL(默认为 [http://127.0.0.1:8090](http://127.0.0.1:8090/)),它将打开 Hasura Console,这是一个帮助你查询索引器 GraphQL API 的 Web UI.
     - [交易流服务](/zh/build/indexer/txn-stream):这是索引器 API 和 SDK 使用的交易 gRPC 流.这仅与你开发 [索引器 SDK](/zh/build/indexer/indexer-sdk) 自定义处理器有关.
     - [Postgres](https://www.postgresql.org/):这是索引器处理器写入的数据库.索引器 API 从这个数据库读取.
     - [水龙头](/zh/build/apis/faucet-api):你可以使用它在本地网络上为账户提供资金.
...
- 你正在构建 [索引器 SDK](/zh/build/indexer/indexer-sdk) 自定义处理器,想要使用全新的网络进行索引.
- 你想清除所有链上状态,例如账户,对象等.
