...
<Steps>
  ...
  5. 等待网络启动
     ...
     - [节点 API](/zh/rest-api):这是直接在节点上运行的 REST API.它支持核心写功能(如交易提交)和有限的读功能,如读取账户资源或 Move 模块信息.
     - [索引器 API](/zh/build/indexer/indexer-api/self-hosted):这是一个 [GraphQL](https://graphql.org/) API,提供对索引区块链数据的丰富读取访问.如果你点击上面的索引器 API URL(默认为 [http://127.0.0.1:8090](http://127.0.0.1:8090)),它将打开 Hasura Console,这是一个帮助你查询索引器 GraphQL API 的 Web UI.
