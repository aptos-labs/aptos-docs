...
它结合了一个快速、低成本且稳定的区块链层，以及 Move 语言的编译时安全特性，可以在部署前捕获潜在漏洞利用（exploit）。此外，Aptos 还提供完善的开发工具（tooling）生态，并与各类交易所和跨链桥深度集成，便于无缝接入。
...
| **FT 标准** | [ERC-20](https://docs.openzeppelin.com/contracts/4.x/erc20) | [Fungible Asset](/zh/build/smart-contracts/fungible-asset) 和 [Coin](/zh/build/smart-contracts/aptos-coin) (旧版) |
...
| | Solidity | Move (Aptos) |
| ---------------------- | ---------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **代币结构** | 每个代币都是独立的合约。 | 每个代币是类型化的 `Coin` 或 `FungibleAsset`，使用单一可复用的合约。 |
| **代币标准** | 必须符合 ERC-20 等标准；实现可能不同。 | 所有代币都有统一的接口和实现。 |
| **余额存储** | 余额使用 `mapping`（映射）结构存储在合约中。 | **面向 Resource 的余额**：余额作为 resource 存储在用户账户中，resource 不能被任意创建，从而确保代币价值的完整性。 |
| **转账机制** | Token 可以在没有接收方明确许可的情况下转移。 | 除特殊情况（如 AptosCoin）外，Token 通常需要接收方的 `signer` 权限才能转移。 |
