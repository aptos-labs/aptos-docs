...
来自 Ethereum？先看站内的 [Ethereum to Aptos 指南](/zh/build/guides/ethereum-to-aptos) 获取分步迁移路径，再把 [Ethereum Cheatsheet](/zh/build/get-started/ethereum-cheatsheet) 作为速查表常驻打开。
...
Aptos 使用安全设计的 [Move](https://aptos-labs.github.io/move-book/) 代替 Solidity。
...
- **无继承。** 合约通过 [friend 模块](https://aptos-labs.github.io/move-book/modules-and-packages.html)进行组合。
- **资源是公开可读的**，通过 REST API，就像 Solidity 存储槽通过 `eth_getStorageAt` 可读一样。不要在链上存储秘密。

参阅 [Move 安全指南](/zh/build/smart-contracts/move-security-guidelines)获取完整的安全参考。要编写和部署您的第一个合约，请按照[您的第一个 Move 模块](/zh/build/guides/first-move-module)操作。Move 还内置了[升级策略](https://aptos-labs.github.io/move-book/cli-deploy.html)，无需代理模式，您可以在准备就绪时将包锁定为 `immutable`。
