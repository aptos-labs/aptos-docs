...
Aptos 是一条用于转移 APT、[稳定币](/zh/build/guides/exchanges#稳定币地址) 和其他[同质化资产](/zh/build/smart-contracts/fungible-asset) 的 Layer 1 结算轨道。支付在交易提交（commit）后即最终确认（BFT 共识，无需额外区块确认）。本页给出规划时应使用的延迟、出块时间和手续费数字，并将这些数字对应到普通转账、机密转账和其他支付流程。
...
这三个数字回答不同的问题。不要把出块时间当成用户等待支付完成的时间。
...
flowchart LR
  ...
  block --> exec["4. 执行"]
  exec --> commit["5. 提交 = 最终确认"]
  commit --> wait["6. 客户端观察到结果"]
```

| 指标             | 衡量内容                                 | 主网典型值                                                                                                                                 | 如何核验                                                                                                  |
| -------------- | ------------------------------------ | ------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------------- |
| **出块时间**       | 相邻区块的间隔                              | 数十毫秒，主网上常低于 50 ms。Aptos 不使用固定槽位时钟                                                                                                     | [区块](/zh/network/blockchain/blocks)、[Explorer](https://explorer.aptoslabs.com/blocks?network=mainnet) |
| **最终性**        | 支付何时不可逆                              | [提交后立即最终](/zh/build/guides/exchanges#交易何时具有终局性)。无需确认深度，也没有重组窗口                                                                        | [交易和状态](/zh/network/blockchain/txns-states)                                                           |
| **端到端（E2E）延迟** | 客户端提交 → 已提交且 `waitForTransaction` 可见 | 正常负载下通常为**亚秒级**。包含到全节点的客户端往返、内存池、共识、执行和轮询                                                                                             | 提交后务必调用 `waitForTransaction`                                                                          |
| **交易手续费**      | 向费用支付者收取的 APT                        | `gas_used × gas_unit_price`（以 [Octa](/zh/network/glossary#octa) 计），并在交易分配新状态时加上[存储费用](/zh/network/blockchain/gas-txn-fee)。**与转账金额无关** | [Gas 和存储费用](/zh/network/blockchain/gas-txn-fee)                                                       |
...
1. 构建、（可选）模拟并签名。
2. 提交到全节点 REST API。返回 202 和哈希表示节点**接受**了交易，并不表示交易已提交上链。
3. 等待交易被提交。各 SDK 通过 `waitForTransaction` 提供这一步。

在 Aptos 上，简单支付的这一等待通常不到一秒。提交**之前**的额外工作（机密资产证明生成、收集多签签名、加密待处理负载）不计入共识时间，却常常主导用户感知时延。

提交（submit）不等于结算。轮询直到 commit，再将支付视为最终。

### 最终性

Aptos 使用 BFT 共识。已提交的交易即最终确认。你不必像在概率最终性链上那样再等若干区块。见[交易所常见问题中的终局性](/zh/build/guides/exchanges#交易何时具有终局性)以及[交易生命周期](/zh/network/blockchain/blockchain-deep-dive)。

### 交易手续费

手续费包含两部分，详见 [Gas 和存储费用](/zh/network/blockchain/gas-txn-fee)：

1. **执行与 IO** — gas 单位 × `gas_unit_price`（Octa）。单价可在负载升高时上涨；更高价格只帮助交易**进入**下一个区块，而不改变它在区块内的顺序。
2. **存储** — 为新建或增大的状态槽支付固定 APT（例如接收方第一次持有某种同质化资产）。当支付创建存储而非更新已有存储时，存储费可能占主导。

同样的负载下，转 1 APT 与转 1,000,000 APT 的 gas 成本几乎相同。主网平均手续费处于**不到 1 美分**的量级（公开网络数据大约为 0.0005 美元）。简单的 APT 和同质化资产转账属于最便宜的交易之列。请始终针对你将提交的准确负载做模拟。
...
| **机密转账**          | 相同的提交路径；执行更重、负载更大 | 在提交**之前**生成零知识证明  | 高于明文转账                                     | [机密资产](/zh/build/smart-contracts/confidential-asset)（不要根据本页实现）                                  |
...
| **加密待处理负载**       | 提交后相同             | 构建时加密             | `gas_unit_price` 最低为 **200** Octa（普通下限的两倍） | [加密待处理交易](/zh/build/guides/encrypted-pending-transactions)                                      |
...
<Aside type="caution">
  第一次向某账户贷记某种 FA 可能分配存储槽，因此会在执行 gas 之外支付[存储费用](/zh/network/blockchain/gas-txn-fee)。请模拟确切的发送方/接收方组合，不要假设与重复转账成本相同。
</Aside>

读取余额请使用 `aptos.getBalance`（APT）或 `0x1::primary_fungible_store::balance` 视图函数。索引与产品表见 [Indexer](/zh/build/indexer) 和[同质化资产余额查询](/zh/build/indexer/indexer-api/fungible-asset-balances)。

## 机密转账

[机密资产（CA）](/zh/build/smart-contracts/confidential-asset) 将同质化资产封装起来，使**金额和机密余额保持隐藏**（包括对验证者），并使用零知识证明。发送方和接收方**地址仍然可见**。入账资金进入待处理机密余额，必须滚动后才能花费。

不要手工构建 CA 交易。使用 [机密资产（SDK）](/zh/build/sdks/ts-sdk/confidential-asset) 中记录的 TypeScript 客户端。该包负责生成证明、解密余额并构造入口函数负载。

相对于明文 FA 转账：

- **E2E 延迟：** 提交后的共识通常仍为亚秒级。客户端上的证明生成发生在提&#x4EA4;_&#x4E4B;前_，这是常见的额外延迟。
