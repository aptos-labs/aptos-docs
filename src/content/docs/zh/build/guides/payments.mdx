...
| 支付           | 赞助                                                           | 无序                                                  | 多代理（双方同意）                                          |
| ------------ | ------------------------------------------------------------ | --------------------------------------------------- | -------------------------------------------------- |
| **APT**      | 费用支付者承担 gas，发送方转出的 APT 不会被手续费扣减                              | 同一热钱包或工资账户并行打款                                      | 自定义双签名模块。不要给 `0x1::aptos_account::transfer` 加二级签名者 |
| **FA / 稳定币** | 典型免 gas 体验：用户只持有 FA，应用支付 APT gas                             | 同样的并行提交                                             | 自定义双签名结算、兑换或货银对付                                   |
| **机密**       | CA 客户端一等支持：`ConfidentialAsset({ withFeePayer: true })` 或按次传入 | 若自行构建 CA 入口函数，可在交易层使用 nonce。高层 CA 客户端记录了费用支付者，未记录无序 | 标准 CA 转账是单方发送。双方同意的机密结算需要双方都签名的自定义合约               |
| **加密待处理**    | 可组合（`withFeePayer: true`）                                    | 可组合（`replayProtectionNonce`）                        | 可组合（`build.multiAgent`）                            |
| **NFT / 对象** | 与 APT 或 FA 相同的费用支付者模式                                        | 同样的并行提交                                             | 双方签名的对象换代币或双向对象转移                                  |
| **金库 / 多签**  | N-of-M 批准后仍可由赞助方支付 gas                                       | 金库并行操作可用无序                                          | 另一套工具：两个账户都必须授权时用多代理，而不是一个账户有多把密钥时用多代理             |
| **无密钥**      | 赞助 gas，新用户无需持有 APT                                           | 与其他发送方相同的 nonce 选项                                  | 无密钥账户可作为发送方或二级签名者参与双方交易                            |
...
- **多代理：** 单方 `aptos_account::transfer` 只需要发送方。若双方必须同意（原子兑换、托管释放、「只有你也签名我才付款」），请使用带两个 `&signer` 参数的自定义入口函数。给 `0x1::aptos_account::transfer` 增加二级签名者会失败（`NUMBER_OF_SIGNER_ARGUMENTS_MISMATCH`）。见[双方同意](#双方同意多代理交易)。
...
- **多代理：** 单方 `primary_fungible_store::transfer` 是付款方向接收方的推送。货银对付（稳定币换 NFT、USDC 换 APT、两种 FA 原子互换）时，两个账户对同一个自定义入口函数签名，任何一方都不能声称对方已经先付了。
...
**[多代理交易](/zh/build/sdks/ts-sdk/building-transactions/multi-agent-transactions)** 是另一种形态：Alice 和 Bob 都对同一笔交易签名。只有列出的每个账户都授权，Move 函数才会运行，因此两条腿要么都发生，要么都不发生。需要双方同意时使用：原子兑换、托管释放、商户必须接受的退款，或任何不应由单方发送的支付。
...
框架里没有已经接受两个签名者的 `0x1::` 支付函数。你需要发布（或复用）带有多个 `&signer` 参数的入口函数，然后：

```typescript filename="two-party-payment.ts"
const transaction = await aptos.transaction.build.multiAgent({
  sender: alice.accountAddress,
  secondarySignerAddresses: [bob.accountAddress],
  withFeePayer: true, // 可选
  data: {
    function: "0xYOUR_MODULE::payments::settle", // 必须接受两个签名者
...
教学模块见 [`two_by_two_transfer`](https://github.com/aptos-labs/aptos-ts-sdk/blob/main/tests/move/transfer/sources/script_two_by_two.move#L5)：每个签名者在同一笔交易中从自己的账户转出 Coin。其他 SDK 使用同样的 `multiAgent` 模式：[Python](/zh/build/sdks/python-sdk/building-transactions/multi-agent-transactions)、[Go](/zh/build/sdks/go-sdk/building-transactions/multi-agent-transactions)、[Rust](/zh/build/sdks/rust-sdk/building-transactions/multi-agent-transactions)。
...
- **多代理：** 单方对象转移是推送。NFT 换稳定币、且任何一方都不应先动时，买家与卖家（或市场托管）对同一个入口函数签名。
