...
| -------------------------- | ------- | ------------ |
| transaction\_version       | bigint! | 交易版本。        |
| transaction\_block\_height | bigint! | 交易所在区块高度。    |
| signer                     | String! | 签名者地址。       |
| is\_sender\_primary        | Boolean | 是否为发送者的主签名。  |
| type                       | String! | 签名类型字符串。     |
| public\_key                | String! | 该签名对应的公钥。    |
| signature                  | String! | 签名的十六进制字节。   |
| threshold                  | bigint  | 多重签名阈值（如适用）。 |
| public\_key\_indices       | jsonb   | 参与签名的密钥索引。   |
| multi\_agent\_index        | bigint  | 二级签名者中的索引。   |
| multi\_sig\_index          | bigint  | 多重签名认证器中的索引。 |

## 机密资产

### `confidential_asset_activities`

[机密资产](/zh/build/smart-contracts/confidential-asset) 的活动（注册、存入、提取、转账、rollover）。金额可能被加密；`event_data` 保存原始事件负载。
...
| from\_address              | String! | 提供代币的地址。            |
| to\_address                | String! | 可领取代币的地址。           |
| amount                     | numeric | 要约数量。               |
| property\_version          | numeric | token v1 属性版本。      |
