---
title: "Go SDK：批量处理交易"
description: "利用 Go SDK 内置的批处理能力，从单个账户并发执行多笔交易"
sidebar:
  label: "批量处理交易"
---

Go SDK 内置了并发发送多笔交易并为其排序的方式。
当需要从同一账户快速执行多笔交易时，这会是一个方便的工具。

如下面的示例所示，可以使用 `client.BuildSignAndSubmitTransactions` 完成此操作。
