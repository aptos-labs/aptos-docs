...
<Aside type="caution" title="旧版钱包 API 已弃用">
  通过浏览器全局变量（如 `window.aptos` 或 `window.petra`）直接访问钱包的方式已**弃用**，不应再使用。这些旧版 API 不再维护，钱包可能随时将其移除。所有 dapp 应使用 [Aptos 钱包适配器](/zh/build/sdks/wallet-adapter) 配合 [AIP-62](https://github.com/aptos-foundation/AIPs/blob/main/aips/aip-62.md) 标准进行钱包集成。
</Aside>
