...
以下表格描述了结构体级别的字段.完整列表请参阅[Aptos代币框架](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-token).

#### 存储在创建者地址的资源

| 字段                                                                                                                                                                    | 描述                                                                                              |   |
| --------------------------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------------------- | - |
| [`Collections`](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-token/token#resource-collections)                                | 维护一个名为`collection_data`的表格,将集合名称映射到`CollectionData`.同时存储该创建者创建的所有`TokenData`.                   |   |
| [`CollectionData`](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-token/token#struct-collectiondata)                            | 存储集合元数据.supply表示当前集合中已创建的代币数量,maximum是该集合中代币数量的上限.                                              |   |
| [`CollectionMutabilityConfig`](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-token/token#0x3_token_CollectionMutabilityConfig) | 指定哪些字段是可变的.                                                                                     |   |
| [`TokenData`](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-token/token#0x3_token_TokenData)                                   | 作为存储代币元数据的主要结构体.Properties字段允许用户添加未在代币数据中定义的自定义属性.用户可以根据`TokenData`铸造更多代币,这些代币共享相同的`TokenData`. |   |
| [`TokenMutabilityConfig`](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-token/token#0x3_token_TokenMutabilityConfig)           | 控制哪些字段是可变的.                                                                                     |   |
| [`TokenDataId`](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-token/token#0x3_token_TokenDataId)                               | 用于表示和查询链上`TokenData`的ID.该ID主要包含三个字段:创建者地址,集合名称和代币名称.                                            |   |
| [`Royalty`](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-token/token#0x3_token_Royalty)                                       | 指定计算版税的分母和分子,并包含用于存放版税的收款人账户地址.                                                                 |   |
...
| 字段                                                                                                                                    | 描述                                                                                                  |
| ------------------------------------------------------------------------------------------------------------------------------------- | --------------------------------------------------------------------------------------------------- |
| [`TokenStore`](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-token/token#0x3_token_TokenStore) | 存储该地址所拥有代币的主要结构体.它将 `TokenId` 映射到实际代币.                                                              |
| [`Token`](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-token/token#0x3_token_Token)           | 数量字段表示代币的个数.                                                                                        |
| [`TokenId`](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-token/token#0x3_token_TokenId)       | `TokenDataId` 指向该代币的元数据.`property_version` 表示该代币的属性相对于 `TokenData` 中 `default_properties` 发生了变异的版本. |

更详细描述请参阅 [Aptos代币框架](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-token).
...
如果用户希望直接接收 NFT 转账,跳过初始的报价和申领步骤,可以调用 [`opt_in_direct_transfer`](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-token/token#0x3_token_opt_in_direct_transfer)
来允许他人直接将 NFT 转入该用户的代币存储库.选择启用直接转账后,用户可以调用 [`transfer`](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-token/token#0x3_token_transfer)
...
发送方和接收方可以共同签署一笔转账交易,直接将代币从发送方转移到接收方 [`direct_transfer_script`](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-token/token#function-direct_transfer_script).例如,当 Alice 和 Bob 都签署了转账交易后,代币将直接从 Alice 的账户转移到 Bob 的账户.
