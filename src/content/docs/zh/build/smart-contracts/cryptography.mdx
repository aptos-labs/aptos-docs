---
title: "密码学"
description: "中文指南：learn about cryptography for move smart contract development on aptos blockchain."
sidebar:
  label: "Cryptography"
---
...
1. **经济的 Gas 使用** – 通过将关键原语实现为 [Move 原生函数](https://aptos-labs.github.io/move-book/functions.html#native-functions),努力为 Move 开发者最小化 gas 成本.例如,参见 [BLS 签名在 BLS12-381 椭圆曲线上的模块](https://github.com/aptos-labs/aptos-core/blob/main/aptos-move/framework/aptos-stdlib/sources/cryptography/bls12381.move).
...
通常,此类应用的开发者需要等待其所需的密码学功能在 [Aptos Move 框架](/zh/build/smart-contracts/why-move) 中作为 [Move 原生函数](https://aptos-labs.github.io/move-book/functions.html#native-functions) 高效实现.
...
例如,Move 开发者可以通过在其实现中使用 [类型参数](https://aptos-labs.github.io/move-book/functions.html#type-parameters) 为曲线类型,通用地在 **任何** 曲线上实现流行的 Boneh-Lynn-Shacham (BLS) 签名方案:
...
如果你需要将 `snarkjs` Groth16 的验证密钥(VK)、证明和公开输入转换为上述 Move 验证器期望的格式,你可以使用这些社区工具:

- [snarkjs-to-aptos](https://github.com/zjma/snarkjs-to-aptos) — 将 `snarkjs` 输出转换为 Aptos 验证器工件.
- [export-aptos-verifier](https://github.com/mysteryon88/export-aptos-verifier) — 导出 `snarkjs` 兼容输出,支持 BN256 和 BLS12-381 曲线,包括 `gnark`、`noname` 与 arkworks 工作流.
- [zk-aptos-examples](https://github.com/zk-examples/zk-aptos-examples) — 包含已生成的 Aptos 智能合约示例.
