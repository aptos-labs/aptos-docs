...
这表示 [无序交易](/zh/build/aips/aip-123) 重用了仍保留在链上 nonce 历史中的 `(sender, nonce)` 对.请选择新的随机 `u64` nonce,或等到旧 nonce 被垃圾回收(约在上一笔交易过期后 130 秒以上).
...
| EMPTY\_PAYLOAD\_PROVIDED                                        |                                                                              交易负载为空.                                                                             | 包含有效的入口函数、脚本或其他负载                                                                                                     |
