---
title: "第三方依赖"
description: "了解 Aptos 区块链上 Move 智能合约开发的第三方依赖"
sidebar:
  label: "Dependencies"
---
...
第三方依赖是受控模块与之交互的外部 [模块](https://aptos-labs.github.io/move-book/modules-and-packages.html).通常,这些外部模块存在于不同的账户下.

## 多 DEX 路由器示例

多 DEX 路由器积极利用第三方依赖.与其提交多笔交易分别与不同 DEX 及其 swap 路径中的各自 [entry](https://aptos-labs.github.io/move-book/functions.html#entry-modifier) 函数交互,不如让一个模块(或 [脚本](https://aptos-labs.github.io/move-book/modules-and-packages.html#scripts))将所有独立的 DEX 调用合并为一笔原子交易.多 DEX 路由器通过引用并调用每个第三方 DEX 模块中的函数来实现这一点.
...
Aptos CLI 支持下载 [包](https://aptos-labs.github.io/move-book/modules-and-packages.html) 的字节码.
