...
- **Apply()**：这是一个确定性函数，对于特定的初始状态和特定交易，始终返回相同的最终状态。如果区块链的当前状态是**S<sub>_i-1_</sub>**，并且在状态**S<sub>_i-1_</sub>**上执行交易**T<sub>_i_</sub>**，那么区块链的新状态始终是**S<sub>_i_</sub>**。Aptos区块链使用[Move语言](/zh/build/smart-contracts/book)来实现确定性执行函数**Apply()**。
