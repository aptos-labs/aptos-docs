...
[使用 CLI 的二进制版本构建](/zh/build/cli),但如果你想从源代码构建 Aptos 节点或 CLI,可以使用以下方法.
...
<details>
  ...
  **> 使用自动化脚本**
  ...
  1. 确保你安装了 `brew` 包管理器:[https://brew.sh/](https://brew.sh/)
  2. 运行开发设置脚本来准备你的环境:
...
如果你已经安装了上述的 Move Prover Tools,那么你就不需要跳过验证器测试.如果你想安装验证器(可选),请遵循[验证器指南](/zh/build/smart-contracts/prover)
