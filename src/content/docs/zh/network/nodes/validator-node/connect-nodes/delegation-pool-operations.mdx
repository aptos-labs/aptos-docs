...
2. 在 Aptos 区块链上[初始化本地配置并创建账户](/zh/build/cli/setup-cli).
...
在委派池收到 100 万 APT,并且验证者成为活跃验证者之前,每个周期内都不会产生可供追踪的奖励.想要查看委派池的详细信息,请使用 Aptos [View function](/zh/rest-api/operations/view):
